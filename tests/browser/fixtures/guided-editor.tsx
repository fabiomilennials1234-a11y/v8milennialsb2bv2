...
const workflowId = params.get('workflow') || 'workflow-1';
createRoot(document.getElementById('root')!).render(
  <QueryClientProvider client={client}><AuthProvider>
    <MemoryRouter initialEntries={[params.has('list') ? '/automacoes' : params.has('new') ? '/automacoes/novo' : `/automacoes/${workflowId}`]}>
      <nav><Link to={`/automacoes/${workflowId}`}>Automação A</Link><Link to="/automacoes/workflow-2">Automação B</Link></nav>
