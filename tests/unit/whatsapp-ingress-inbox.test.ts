...
it('does not complete durable work before its background task settles', async () => {
  ...
  expect(rpc).toHaveBeenCalledWith('finish_whatsapp_ingress_event',{p_id:'event-id',p_lease_token:'lease',p_error_code:null});
...
it.each([500,503])('retains HTTP %i as retryable work, not completion', async status => {
  ...
  expect(rpc).toHaveBeenCalledWith('finish_whatsapp_ingress_event',expect.objectContaining({p_error_code:`http_${status}`}));
});
...
it('marks rejected background work retryable even if synchronous handler returned 200', async () => {
  ...
  expect(rpc).toHaveBeenCalledWith('finish_whatsapp_ingress_event',expect.objectContaining({p_error_code:'background_failed'}));
});
