...
BEGIN
  ...
  monthly_revenue AS (
    SELECT
      date_trunc('month', pp.closed_at)::date AS m,
      COALESCE(SUM(pp.sale_value), 0) AS rev
    FROM pipe_propostas pp
    WHERE pp.organization_id = p_org_id
      AND pp.status = 'vendido'
      AND pp.closed_at >= (p_end_date - interval '3 months')::date
      AND pp.closed_at <= p_end_date
      AND (p_member_id IS NULL OR pp.closer_id = p_member_id)
    GROUP BY date_trunc('month', pp.closed_at)::date
    ORDER BY m
  ),
  revenue_trend AS (
    SELECT
