-- 20270919000030 — SCRUM-647, fatia 2 (arquivo 2 de 2)
-- get_dashboard_metrics passa a ler a projecao canonica.
--
-- POR QUE SOZINHA NUM ARQUIVO, e nao junto com as outras 15
-- A regra `ledger-revenue` do scripts/check-metric-antipatterns.sh e de
-- ARQUIVO, nao de linha: `if ! grep -qi sale_events "$file"` isenta o arquivo
-- INTEIRO. get_dashboard_metrics le `sale_events` de verdade (as vendas vem do
-- caderno desde a decisao do CTO registrada no proprio corpo) — a isencao e
-- legitima PARA ELA. Mas colocar este corpo no mesmo arquivo das outras 15
-- desligaria a regra para as 15 tambem, de graca e em silencio.
--
-- Um arquivo por isencao mantem o gate honesto: aqui a regra esta desligada
-- porque a funcao le o caderno; la ela continua ligada.
--
-- A TROCA: `FROM pipe_propostas WHERE organization_id = ...` vira
-- `FROM negocio_projetado WHERE funil_sistema = 'propostas' AND
-- organization_id = ...`. E o unico ponto do corpo que toca uma view de compat;
-- o resto e byte-a-byte o de prod (pg_get_functiondef, 2026-09-03). As colunas
-- lidas pelo predicado (metrics_period_at, created_at, sale_responsible_id,
-- closer_id, responsible_id) tem o MESMO nome e o MESMO cast na projecao.
--
-- CREATE OR REPLACE: assinatura e grants preservados, sem DROP.
-- ROLLBACK: supabase/migrations/rollback/20270919000030_*.sql (corpo de prod).
