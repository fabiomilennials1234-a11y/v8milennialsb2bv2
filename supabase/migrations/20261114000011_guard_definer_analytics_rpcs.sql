-- 20261114000001_guard_definer_analytics_rpcs.sql
--
-- INCIDENTE DE SEGURANCA 2026-06-01 (2a classe) — vazamento cross-tenant via RPCs SECURITY DEFINER.
--
-- 10 RPCs de analytics/dashboard recebiam p_org_id e NAO checavam se o caller pertence a org.
-- DEFINER bypassa RLS => qualquer usuario autenticado lia o comercial de QUALQUER org
-- (MRR, receita, leads, vendedores, produtos, funil). Provado em prod: dna de almas lia a
-- Milennials. Alem disso get_operations_overview/get_usage_by_org agregavam TODAS as orgs.
--
-- Fix: helper assert_org_access(p_org_id) [service_role OU master OU membro] chamado no topo
-- de cada RPC; operations/usage gated por is_master_user(). Verificado: nao-membro -> access_denied,
-- membro/master -> dados normais. Aplicado out-of-band no prod durante o incidente; este e o registro.
--
-- NOTA: get_portfolio_clients/kpis, get_revenue_at_risk, get_vendedor_ranking JA tinham guard
-- (assert_org_member) e nao foram tocadas.
...
CREATE OR REPLACE FUNCTION public.get_portfolio_trends(p_org_id uuid, p_months integer DEFAULT 6)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_usage_by_org(interval_param text DEFAULT '7 days'::text)
 RETURNS TABLE(organization_id uuid, organization_name text, total_events bigint, leads_criados bigint, cards_movidos bigint, mensagens_enviadas bigint, ultima_atividade timestamp with time zone)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT o.id, o.name, COUNT(*) AS total_events,
  ...
  FROM public.usage_events u JOIN public.organizations o ON o.id = u.organization_id
  WHERE public.is_master_user() AND u.created_at > NOW() - interval_param::INTERVAL
  GROUP BY o.id, o.name ORDER BY ultima_atividade DESC;
$function$;

GRANT EXECUTE ON FUNCTION public.assert_org_access(uuid) TO authenticated, anon, service_role;
