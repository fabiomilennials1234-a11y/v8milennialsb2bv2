-- Emergency scoped fix: Agafarma in Chique Distribuidora explicitly enabled
-- reenrollment, but the legacy max_times/cooldown fields silently suppress it.
-- Preserve the existing guard for every other workflow and organization.
-- Remove this exception only after the general 20271021000022 policy is
-- reviewed and intentionally deployed across tenants.
...
BEGIN
  ...
  -- This workflow is the only approved exception. Keep the explicit toggle
  -- authoritative; no special treatment when reenrollment is disabled.
