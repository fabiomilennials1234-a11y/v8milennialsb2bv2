-- Repair create_lead_with_pipe after leads.meeting_date was removed.
-- Keep the public signature intact because both lead webhooks call it. Meeting
-- time continues to live in pipeline metadata; compromisso_date remains the
-- only meeting-like column on leads.
...
BEGIN
  ...
  INSERT INTO public.leads (
    ...
    segment, faturamento, urgency, responsible_id, compromisso_date,
    ...
    p_segment, p_faturamento, p_urgency, p_responsible_id, p_compromisso_date,
  ...
  ELSIF p_pipe_type = 'confirmacao' THEN
    v_pipe_id := public.fn_entrada_sistema_criar(
      ...
      p_metadata        => jsonb_build_object(
        ...
        'responsible_id',    COALESCE(p_pipe_responsible_id, p_sdr_id, p_closer_id), -- metric-lint-allow: compatibility metadata from the existing webhook contract.
        'sdr_id',            p_sdr_id,
...
COMMENT ON FUNCTION public.create_lead_with_pipe IS
  'Cria lead e posição de funil atomicamente usando apenas colunas vivas de leads; mantém a assinatura legada para os webhooks.';

REVOKE ALL ON FUNCTION public.create_lead_with_pipe(text,text,text,text,text,text,uuid,uuid,uuid,integer,text,text,text,text,uuid,timestamptz,timestamptz,text,text,text,text,text,text,text,timestamptz,text,uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_lead_with_pipe(text,text,text,text,text,text,uuid,uuid,uuid,integer,text,text,text,text,uuid,timestamptz,timestamptz,text,text,text,text,text,text,text,timestamptz,text,uuid) TO authenticated, service_role;
