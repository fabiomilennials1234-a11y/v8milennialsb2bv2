-- Explicit custom-definition scopes; preserve legacy readers/publication for recovery.
BEGIN;
...
AS $$
  -- Defensive execution budget; final representative-load certification is ticket 21.
...
ALTER TABLE public.workflow_data_grants ADD CONSTRAINT workflow_data_grants_fields_check
  CHECK (public.valid_guided_data_scopes(fields));
...
BEGIN
  ...
  PERFORM 1 FROM public.lead_custom_fields f WHERE f.id = ANY(v_new_custom_ids)
    AND f.organization_id = v_organization_id AND f.field_type = 'text' FOR SHARE;
  IF (SELECT count(*) FROM public.lead_custom_fields f WHERE f.id = ANY(v_new_custom_ids)
    AND f.organization_id = v_organization_id AND f.field_type = 'text') <> cardinality(v_new_custom_ids) THEN
...
BEGIN
  ...
  IF jsonb_typeof(p_definition) IS DISTINCT FROM 'object'
    ...
    OR NOT (p_required_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.segment','lead.urgency','lead.faturamento','lead.tags','lead.origin','lead.pre_sale_responsible_id','lead.sale_responsible_id']::text[]) THEN
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 13
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.segment','lead.urgency','lead.faturamento']::text[]) THEN
    RAISE EXCEPTION 'invalid_scope' USING ERRCODE = '22023';
  END IF;
  SELECT w.organization_id INTO v_organization_id
  ...
  RETURN QUERY SELECT l.id, l.organization_id,
    ...
    || (CASE WHEN 'lead.segment' = ANY(p_fields) THEN jsonb_build_object('segment', l.segment) ELSE '{}'::jsonb END)
    || (CASE WHEN 'lead.urgency' = ANY(p_fields) THEN jsonb_build_object('urgency', l.urgency) ELSE '{}'::jsonb END)
    || (CASE WHEN 'lead.faturamento' = ANY(p_fields) THEN jsonb_build_object('faturamento', l.faturamento) ELSE '{}'::jsonb END)
...
BEGIN
  ...
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 14
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.segment','lead.urgency','lead.faturamento','lead.tags']::text[])
...
CREATE OR REPLACE FUNCTION public.read_guided_condition_data(
  p_workflow_id uuid, p_organization_id uuid, p_lead_id uuid, p_fields text[], p_tag_ids uuid[], p_origin_ids uuid[]
) RETURNS TABLE(id uuid, organization_id uuid, field_values jsonb)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE v_organization_id uuid; v_tag_ids uuid[]; v_origin_ids uuid[];
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 15
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.segment','lead.urgency','lead.faturamento','lead.tags','lead.origin']::text[])
...
END;
$$;
REVOKE ALL ON FUNCTION public.read_guided_condition_data(uuid,uuid,uuid,text[],uuid[],uuid[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.read_guided_condition_data(uuid,uuid,uuid,text[],uuid[],uuid[]) TO service_role;
CREATE OR REPLACE FUNCTION public.read_guided_condition_data(
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 17
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.segment','lead.urgency','lead.faturamento','lead.tags','lead.origin','lead.pre_sale_responsible_id','lead.sale_responsible_id']::text[])
...
CREATE OR REPLACE FUNCTION public.read_guided_condition_custom_data(
  p_workflow_id uuid, p_organization_id uuid, p_lead_id uuid, p_fields text[], p_tag_ids uuid[], p_origin_ids uuid[], p_member_ids uuid[]
) RETURNS TABLE(id uuid, organization_id uuid, field_values jsonb)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE v_organization_id uuid; v_tag_ids uuid[]; v_origin_ids uuid[]; v_member_ids uuid[]; v_custom_ids uuid[];
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF NOT public.valid_guided_data_scopes(p_fields) OR cardinality(p_fields) = 0
    OR p_member_ids IS NULL OR array_position(p_member_ids, NULL) IS NOT NULL
    OR (cardinality(p_member_ids) > 0 AND NOT (p_fields && ARRAY['lead.pre_sale_responsible_id','lead.sale_responsible_id']::text[]))
    OR p_origin_ids IS NULL OR array_position(p_origin_ids, NULL) IS NOT NULL
    OR (cardinality(p_origin_ids) > 0 AND NOT ('lead.origin' = ANY(p_fields)))
  ...
  PERFORM 1 FROM public.leads l WHERE l.id = p_lead_id
    AND l.organization_id = v_organization_id AND l.deleted_at IS NULL FOR SHARE;
  IF NOT FOUND THEN RAISE EXCEPTION 'context_unavailable' USING ERRCODE = 'PT404'; END IF;
  SELECT coalesce(array_agg(DISTINCT substr(scope, 13)::uuid), '{}'::uuid[]) INTO v_custom_ids
    FROM unnest(p_fields) AS requested(scope) WHERE scope LIKE 'lead.custom:%';
  IF cardinality(v_custom_ids) = 0 THEN RAISE EXCEPTION 'invalid_scope' USING ERRCODE = '22023'; END IF;
  PERFORM 1 FROM public.lead_custom_fields f WHERE f.id = ANY(v_custom_ids)
    AND f.organization_id = v_organization_id FOR SHARE;
  IF (SELECT count(*) FROM public.lead_custom_fields f WHERE f.id = ANY(v_custom_ids)
    AND f.organization_id = v_organization_id) <> cardinality(v_custom_ids) THEN
    RAISE EXCEPTION 'reference_unavailable' USING ERRCODE = 'PT422';
  END IF;
  SELECT coalesce(array_agg(DISTINCT requested.id), '{}'::uuid[]) INTO v_tag_ids
    FROM unnest(p_tag_ids) AS requested(id);
  ...
  RETURN QUERY SELECT l.id, l.organization_id,
    ...
    || jsonb_build_object('custom_fields', (
      SELECT jsonb_agg(jsonb_build_object('id', f.id, 'name', f.field_name, 'field_type', f.field_type, 'value', v.value) ORDER BY f.id)
      FROM public.lead_custom_fields f
      LEFT JOIN public.lead_custom_field_values v ON v.field_id = f.id AND v.lead_id = l.id
      WHERE f.id = ANY(v_custom_ids) AND f.organization_id = v_organization_id
    ))
    FROM public.leads l WHERE l.id = p_lead_id AND l.organization_id = v_organization_id AND l.deleted_at IS NULL;
END;
$$;
REVOKE ALL ON FUNCTION public.read_guided_condition_custom_data(uuid,uuid,uuid,text[],uuid[],uuid[],uuid[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.read_guided_condition_custom_data(uuid,uuid,uuid,text[],uuid[],uuid[],uuid[]) TO service_role;
