-- Personal custom-field evaluation uses one caller-RLS snapshot of definitions
-- and answers. Missing definitions are errors; missing answers remain null.
BEGIN;
...
BEGIN
  ...
  IF p_field_ids IS NULL OR cardinality(p_field_ids) = 0 OR array_position(p_field_ids, NULL) IS NOT NULL THEN
