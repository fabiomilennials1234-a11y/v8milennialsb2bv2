-- Validate responsible references before publishing; retain the complete current reader/grant definitions for recovery.
...
BEGIN
  ...
  IF jsonb_typeof(p_definition) IS DISTINCT FROM 'object'
    ...
    OR NOT (p_required_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.tags','lead.origin','lead.pre_sale_responsible_id','lead.sale_responsible_id']::text[]) THEN
  ...
  ) SELECT coalesce(jsonb_agg(jsonb_build_object('nodeId', node_id, 'originId', CASE WHEN condition ->> 'operator' = 'is_empty' THEN NULL ELSE condition ->> 'originId' END)), '[]'::jsonb)
  ...
  WITH RECURSIVE conditions(node_id, condition) AS (
    ...
    SELECT c.node_id, child FROM conditions c CROSS JOIN LATERAL jsonb_array_elements(
      ...
      'requiresReference', condition ->> 'operator' IS DISTINCT FROM 'is_empty',
      'memberId', CASE WHEN condition ->> 'operator' = 'is_empty' THEN NULL ELSE condition ->> 'memberId' END)), '[]'::jsonb)
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 10
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term']::text[]) THEN
    RAISE EXCEPTION 'invalid_scope' USING ERRCODE = '22023';
  END IF;
  SELECT w.organization_id INTO v_organization_id
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 11
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.tags']::text[])
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 12
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.tags','lead.origin']::text[])
