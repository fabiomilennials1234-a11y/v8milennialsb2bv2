-- Live definitions captured from production 2026-09-24. Scheduler-only probes.
-- Preserve schedules, payloads, auth, worker claims and retry/recovery behavior.
-- New work after a negative probe runs on the next existing tick.
-- No workflow default-worker guard: it also owns recovery and reconciliation.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.meta_asset_bindings WHERE asset_type = 'page' AND status = 'active'
  ) THEN
    RETURN;
  END IF;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.copilot_agent_followup_rules WHERE is_active = true AND trigger_type IN ('no_response', 'after_qualification', 'after_meeting_scheduled', 'post_sale', 'proposal_no_response')
  ) THEN
    RETURN;
  END IF;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.follow_up_automations WHERE is_active = true AND trigger_type <> 'stage_change' AND organization_id IS NOT NULL
  ) THEN
    RETURN;
  END IF;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.copilot_followup_situation_config WHERE is_active = true
  ) THEN
    RETURN;
  END IF;
  SELECT value INTO secret_val FROM public.cron_config WHERE key = 'cron_secret';
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.outbound_dispatch_log WHERE status = 'pending' AND agent_id IS NOT NULL AND scheduled_at <= now()
  ) THEN
    RETURN;
  END IF;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.workflows WHERE is_active = true AND trigger_type IN ('cron', 'scheduled_date', 'lead_no_reply', 'meeting_not_confirmed', 'followup_overdue')
  ) THEN
    RETURN;
  END IF;
