-- Full master parity for guided administration only; restricted master is not full master.
BEGIN;
CREATE OR REPLACE FUNCTION public.can_administer_guided_workflow(p_organization_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT auth.uid() IS NOT NULL AND (
    p_organization_id IN (SELECT public.get_my_admin_organization_ids())
    OR EXISTS (
...
$$;
REVOKE ALL ON FUNCTION public.can_administer_guided_workflow(uuid)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.can_administer_guided_workflow(uuid) TO authenticated;
...
CREATE POLICY workflow_data_grants_admin_read ON public.workflow_data_grants
  FOR SELECT TO authenticated USING (public.can_administer_guided_workflow(organization_id));
