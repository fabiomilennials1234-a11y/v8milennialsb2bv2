-- Add explicit tag grants and an atomic authorized data reader.
-- Includes 13 definitions for superset recovery; existing approvals remain unchanged.
BEGIN;
...
ALTER TABLE public.workflow_data_grants ADD CONSTRAINT workflow_data_grants_fields_check CHECK (
  fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.tags']::text[] AND array_position(fields, NULL) IS NULL AND cardinality(fields) <= 5
);
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.tags']::text[])
    OR array_position(p_fields, NULL) IS NOT NULL OR cardinality(p_fields) > 5
...
BEGIN
  ...
  IF jsonb_typeof(p_definition) IS DISTINCT FROM 'object'
    ...
    OR NOT (p_required_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone']::text[]) THEN
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 4
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone']::text[]) THEN
    RAISE EXCEPTION 'invalid_scope' USING ERRCODE = '22023';
  END IF;
  SELECT w.organization_id INTO v_organization_id
...
BEGIN
  ...
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 5
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.tags']::text[])
