...
RETURNS TABLE (
  ...
  online_users jsonb, measured_at timestamptz
)
...
BEGIN
  ...
  SELECT o.id, o.name, o.slug,
    (SELECT count(*) FROM public.leads l
     WHERE l.organization_id = o.id
       ...
       AND l.deleted_at IS NULL AND l.is_shadow IS NOT TRUE),
  ...
  JOIN public.organizations o ON o.id = go.organization_id
...
COMMENT ON FUNCTION public.gestor_organization_overview() IS
  'Gestor ativo: somente organizações vinculadas; leads criados e vendas líquidas de estornos nas últimas 168h; membros ativos com aba visível nos últimos 2min. Não concede acesso master.';
