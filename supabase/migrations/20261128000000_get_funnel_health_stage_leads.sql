-- Saúde do Funil — drill-down: leads de uma etapa da coorte.
-- Mesma semântica da get_funnel_health (CONTEXT.md: Funnel Health Indicator).
...
REVOKE ALL ON FUNCTION public.get_funnel_health_stage_leads(uuid, timestamptz, timestamptz, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_funnel_health_stage_leads(uuid, timestamptz, timestamptz, text) TO authenticated, service_role;
