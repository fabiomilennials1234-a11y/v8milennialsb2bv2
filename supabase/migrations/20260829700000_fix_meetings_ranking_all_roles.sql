-- Fix meetings ranking: count meetings where member appears as responsible_id, sdr_id, OR closer_id.
-- Previously only COALESCE(responsible_id, sdr_id) was used, which misses members who appear
-- only as closer_id or whose sdr_id is masked by a different responsible_id.
...
BEGIN
  ...
  -- Expand each meeting into rows for each role (responsible, sdr, closer),
  -- then count DISTINCT meetings per member to avoid double-counting.
