-- ===========================================================================
-- ROLLBACK — 20270821170000_metric_ganho_perda.sql (SCRUM-391)
-- ===========================================================================
...
-- dois statements.
--
-- O recorte `desfecho` sai por último e SÓ se ninguém mais o usar — o
-- vocabulário é global, e apagá-lo enquanto outra medida o declara derrubaria
-- aquela medida junto.
-- ===========================================================================

-- 1 — Despachante volta ao corpo de 20270821120000 (19 ramos, sem ganho_perda).
...
-- 2 — A função da medida deixa de existir.
DROP FUNCTION IF EXISTS public._metric_leaf_ganho_perda(uuid, text, tstzrange, text, jsonb);

-- 3 — Catálogo.
DELETE FROM public.metric_catalog_measure_formats  WHERE measure_id = 'ganho_perda';
DELETE FROM public.metric_catalog_measure_recortes WHERE measure_id = 'ganho_perda';
DELETE FROM public.metric_catalog_measures         WHERE id = 'ganho_perda';

-- 4 — Recorte, só se ficou órfão.
DELETE FROM public.metric_catalog_recortes r
 WHERE r.id = 'desfecho'
   AND NOT EXISTS (
     SELECT 1 FROM public.metric_catalog_measure_recortes mr WHERE mr.recorte_id = r.id
   );
