-- Rollback de 20270730000008_voip_reserve_inbound_requires_tc_call_id.sql
--
-- Devolve fn_voip_call_reserve ao corpo vigente em
-- 20270730000006_voip_call_id_provenance.sql — cópia LITERAL daquele arquivo,
-- ou seja, com a cunhagem do tc_call_id na reserva de saída preservada e SEM o
-- `AND tc_call_id IS NOT NULL` no ramo de entrada.
--
-- CONSEQUÊNCIA DE RODAR ISTO: volta o efeito colateral que a migration
-- conserta — atender uma chamada de entrada sem id de rede grava
-- `operator_user_id` na linha ANTES de o CRM negar, e o operador fica preso no
-- UNIQUE parcial `idx_voip_calls_one_live_per_operator` até que o varredor
-- `voip-sweep-stuck-calls` (20270730000007) o solte, dois minutos depois. Se os
-- dois rollbacks forem aplicados juntos, o operador não é solto nunca.
--
