-- ROLLBACK de 20270730000009_voip_consentimento_opcional.sql
--
-- Devolve o consentimento de voz à condição de trava dura: chamada de saída
-- volta a EXIGIR uma linha viva em `consent_records`, para toda organização,
-- independente de `require_voice_consent`.
--
-- ANTES DE RODAR ISTO, SAIBA O QUE ACONTECE
-- -----------------------------------------
-- Em 2026-07-31 a produção tinha ZERO linhas de `voice_call_whatsapp`, e
-- `fn_voip_consent_record` — a única porta de entrada — não tem um chamador
-- sequer no produto. Reverter sem antes construir essa porta faz TODA ligação
-- de saída voltar a ser recusada com `consent_missing`, em todas as
-- organizações. Não é hipótese: era exatamente o estado que motivou a mudança.
--
-- A coluna `require_voice_consent` é DEIXADA NO LUGAR de propósito. Removê-la
-- quebraria o `call-plane.ts`, que a lê; e mantê-la é inofensivo, porque a
-- função revertida ignora o valor. Se a intenção for apagar o mecanismo
-- inteiro, o `DROP COLUMN` vai numa migration própria, depois de o TypeScript
-- parar de consultá-la.
--
-- Corpo: cópia literal de 20270730000008 (a definição anterior a esta), com o
-- bloco de consentimento incondicional restaurado.
