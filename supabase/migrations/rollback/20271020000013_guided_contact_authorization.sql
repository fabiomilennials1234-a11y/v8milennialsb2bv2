-- Stop guided admission before rollback. Preserve contact grants/versions/pins.
-- Restore name/company-only writer, finalizer and reader; retain broad storage
-- CHECK to preserve historical contact approvals. Revocation remains available.
-- For full recovery reapply 00..11, then 13 directly (13 includes 12 definitions).
-- Reapplying 12's narrower CHECK to existing contact approvals would fail.
BEGIN;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR NOT (p_fields <@ ARRAY['lead.name','lead.company']::text[])
    OR array_position(p_fields, NULL) IS NOT NULL OR cardinality(p_fields) > 2
...
BEGIN
  ...
  IF jsonb_typeof(p_definition) IS DISTINCT FROM 'object'
    ...
    OR NOT (p_required_fields <@ ARRAY['lead.name','lead.company']::text[]) THEN
...
BEGIN
  ...
  IF p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 2
    ...
    OR NOT (p_fields <@ ARRAY['lead.name','lead.company']::text[]) THEN
    RAISE EXCEPTION 'invalid_scope' USING ERRCODE = '22023';
  END IF;
  SELECT w.organization_id INTO v_organization_id
