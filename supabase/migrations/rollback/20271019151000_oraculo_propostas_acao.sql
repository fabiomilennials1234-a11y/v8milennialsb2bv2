DROP FUNCTION IF EXISTS public.execute_oraculo_action_proposal(uuid,uuid);
DROP FUNCTION IF EXISTS public.oraculo_save_turn(uuid,uuid,uuid,timestamptz,text,jsonb,text);
DROP FUNCTION IF EXISTS public.oraculo_preview_action_proposal(uuid,uuid,text,jsonb,jsonb);
DROP FUNCTION IF EXISTS public.oraculo_action_candidates(uuid,uuid,jsonb,uuid);
DROP TABLE IF EXISTS public.oraculo_action_proposals;
DELETE FROM public.feature_permissions WHERE key = 'followups.create';

-- Restaura a fronteira atômica anterior, sem persistência de propostas.
