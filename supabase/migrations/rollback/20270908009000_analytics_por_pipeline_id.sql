-- rollback/20270908009000_analytics_por_pipeline_id.sql — SCRUM-631
--
-- Desfaz a resolução por pipeline_id das 4 RPCs de análise: derruba as
-- assinaturas novas e restaura os functiondefs EXATOS de prod
-- (pg_get_functiondef, snapshot 2026-09-02, projeto jsjsmuncfkbsbzqzqhfq),
-- com a ACL original medida na mesma data:
--   get_funnel_conversion / get_pipeline_velocity / get_sales_cycle_analysis:
--     anon + authenticated + service_role
--   get_analytics_pipeline_metrics: authenticated + service_role (sem anon)

DROP FUNCTION IF EXISTS public.get_funnel_conversion(text, timestamptz, timestamptz, uuid, uuid);
DROP FUNCTION IF EXISTS public.get_pipeline_velocity(text, timestamptz, timestamptz, uuid, uuid);
DROP FUNCTION IF EXISTS public.get_sales_cycle_analysis(text, timestamptz, timestamptz, uuid, uuid);
DROP FUNCTION IF EXISTS public.get_analytics_pipeline_metrics(uuid, date, date, text, uuid, uuid);
...
REVOKE ALL ON FUNCTION public.get_funnel_conversion(text, timestamptz, timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_funnel_conversion(text, timestamptz, timestamptz) TO anon, authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.get_pipeline_velocity(text, timestamptz, timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_pipeline_velocity(text, timestamptz, timestamptz) TO anon, authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.get_sales_cycle_analysis(text, timestamptz, timestamptz, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_sales_cycle_analysis(text, timestamptz, timestamptz, uuid) TO anon, authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.get_analytics_pipeline_metrics(uuid, date, date, text, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_analytics_pipeline_metrics(uuid, date, date, text, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_analytics_pipeline_metrics(uuid, date, date, text, uuid) TO authenticated, service_role;
