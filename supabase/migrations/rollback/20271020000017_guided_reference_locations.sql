-- Restore reference rejection without node details; preserve all stored data.
BEGIN;
...
BEGIN
  ...
  WITH RECURSIVE conditions(condition) AS (
    SELECT n -> 'data' -> 'guidedCondition'
      FROM jsonb_array_elements(p_definition -> 'nodes') AS n WHERE n ->> 'type' = 'condition'
    UNION ALL
    SELECT child FROM conditions c CROSS JOIN LATERAL jsonb_array_elements(
      CASE WHEN c.condition ->> 'kind' = 'group' THEN c.condition -> 'children' ELSE '[]'::jsonb END
    ) AS child
  ) SELECT coalesce(array_agg(DISTINCT (condition ->> 'tagId')::uuid), '{}'::uuid[]) INTO v_tag_ids
    FROM conditions WHERE condition ->> 'field' = 'lead.tags';
