-- 20270919000020_leitoras_pela_projecao_canonica.sql — ROLLBACK
--
-- Restaura os 15 corpos EXATAMENTE como estavam em PROD em 2026-09-03,
-- capturados com `pg_get_functiondef` do projeto jsjsmuncfkbsbzqzqhfq. Nao e
-- reconstrucao a partir da migration que os criou nem copia do repo: e o corpo
-- VIVO do dia da captura — que e o que importa, porque o repo ja provou estar
-- atras do prod.
--
-- Todas sao CREATE OR REPLACE: assinatura, volatilidade, search_path e grants
-- voltam ao estado anterior sem nenhum DROP, entao nao ha EXECUTE a redeclarar.
--
-- Estas funcoes voltam a ler as 6 views de compat. Isso e proposital: enquanto
-- as views existirem (elas caem na SCRUM-639), o estado anterior e valido.
...
AS $function$
  SELECT
    ...
    AND EXISTS (
      SELECT 1 FROM custom_pipe_entries cpe
      WHERE cpe.lead_id = p_lead_id AND cpe.organization_id = p_org_id
    )
    AND NOT EXISTS (
      SELECT 1 FROM pipe_whatsapp pw
      WHERE pw.lead_id = p_lead_id AND pw.organization_id = p_org_id
    )
    AND NOT EXISTS (
      SELECT 1 FROM pipe_confirmacao pc
      WHERE pc.lead_id = p_lead_id AND pc.organization_id = p_org_id
    )
    AND NOT EXISTS (
      SELECT 1 FROM pipe_propostas pp
      WHERE pp.lead_id = p_lead_id AND pp.organization_id = p_org_id
    );
$function$;
