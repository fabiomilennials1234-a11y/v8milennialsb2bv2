-- Restore text/number support without removing any data or approvals.
-- Authorize and publish current custom text/number definitions by UUID.
BEGIN;
...
BEGIN
  ...
  PERFORM 1 FROM public.lead_custom_fields f WHERE f.id = ANY(v_new_custom_ids)
    AND f.organization_id = v_organization_id AND f.field_type IN ('text', 'number') FOR SHARE;
  IF (SELECT count(*) FROM public.lead_custom_fields f WHERE f.id = ANY(v_new_custom_ids)
    AND f.organization_id = v_organization_id AND f.field_type IN ('text', 'number')) <> cardinality(v_new_custom_ids) THEN
  ...
  IF jsonb_array_length(v_custom_references) > 0 THEN
    SELECT array_agg(DISTINCT r ->> 'nodeId' ORDER BY r ->> 'nodeId') INTO v_invalid_node_ids
      ...
      WHERE r ->> 'fieldId' IS NULL OR r ->> 'fieldId' !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        OR coalesce(r ->> 'fieldType', '') NOT IN ('text', 'number');
