-- SP-0 fix: soft-delete leak — get_analytics_overview_metrics + get_analytics_financial_metrics não filtravam leads.deleted_at (lixeira) nem is_shadow, então leads deletados/shadow ainda contavam. Adiciona AND (l.deleted_at IS NULL) AND (l.is_shadow IS NULL OR l.is_shadow = false) em cada scan de leads, espelhando get_dashboard_metrics (20261125000001). Ver docs/superpowers/specs/2026-07-02-metrics-foundation-design.md
