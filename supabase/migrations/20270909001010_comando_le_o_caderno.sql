-- C3 — o Comando lê o CADERNO de vendas, não o funil.
--
-- Decisão do CTO: o desfecho é do NEGÓCIO, o funil sai de cena, e o Comando
-- passa a contar venda de TODOS os funis. Ver .specs/agenda-fonte-unica/PLANO.md.
--
-- ── O que muda, medido em 6 meses ────────────────────────────────────────
--                     HOJE (funil)      CADERNO        diferença
--   vendas                     361            428            +67
--   receita              R$ 738.370     R$ 869.381    +R$ 131.011
--   MRR                  R$ 324.353     R$ 349.456     +R$ 25.103
--   Projeto              R$ 106.728     R$ 110.128      +R$ 3.400
--   sem classificação    R$ 307.289     R$ 335.046
--                             (42%)          (39%)
--
-- Todos os números SOBEM, e sobem porque o caderno enxerga venda que o Comando
-- não via — as de outros funis. Não é inflação: é o recorte que faltava.
-- O time VAI notar o salto; ele é explicável e precisa ser avisado, não
-- descoberto.
--
-- ── 🚨 `vendaSemClassificacao` é campo NOVO, por decisão explícita ───────
-- 280 de 428 vendas não têm item de produto. São R$ 335 mil que MRR + Projeto
-- não somam, e que hoje EVAPORAM entre três caixas do painel que não fecham.
--
-- Havia três saídas e a escolhida foi a do meio:
--   · deixar como está — três números que não fecham, sem explicação;
--   · esconder o resto — apaga a evidência do problema;
--   · MOSTRAR a terceira fatia — a tela diz a verdade sobre a própria
--     incerteza, e vira pauta para a operação preencher o produto.
--
-- É a mesma escolha da trava de valor (20270909000010): número que não fecha e
-- não explica é pior que número incompleto e honesto. E a causa é a mesma
-- família — venda fechada sem informar o QUE foi vendido, ao lado da venda
-- fechada sem informar QUANTO.
--
-- ── O que NÃO muda ───────────────────────────────────────────────────────
-- `propostasEnviadas` continua vindo de `pipe_propostas`: proposta enviada é
-- conceito de FUNIL, não de venda, e o caderno não tem essa noção. Trocar por
-- trocar aqui seria perder o número sem ganhar nada.
--
-- Leads, reuniões e tempo de resposta seguem intocados.
--
-- ── Fidelidade ───────────────────────────────────────────────────────────
-- Corpo extraído de `pg_get_functiondef`, com quatro blocos emendados
-- (contagem de vendas, receita+mix, dailySales, campo novo no retorno). O
-- resto é byte a byte o que está em produção.
--
-- Reaplicar é no-op.
