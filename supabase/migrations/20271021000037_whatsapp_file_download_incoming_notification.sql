-- FileDownloaded is informational for both incoming and outgoing media.
-- Keep SQL36 immutable; accept only an explicit JSON boolean IsFromMe.
...
BEGIN
  ...
  IF v_event->>'Type' IS DISTINCT FROM 'FileDownloaded'
    OR pg_catalog.jsonb_typeof(v_event->'IsFromMe') IS DISTINCT FROM 'boolean'
  ...
  IF pg_catalog.jsonb_array_length(v_ids) <> 1
    ...
    OR pg_catalog.jsonb_typeof(v_event->'Chat') IS DISTINCT FROM 'string'
