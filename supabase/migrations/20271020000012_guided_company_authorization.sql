-- Add company scope without modifying existing approvals or published versions.
BEGIN;
...
ALTER TABLE public.workflow_data_grants ADD CONSTRAINT workflow_data_grants_fields_check CHECK (
  fields <@ ARRAY['lead.name','lead.company']::text[] AND array_position(fields, NULL) IS NULL AND cardinality(fields) <= 2
);
