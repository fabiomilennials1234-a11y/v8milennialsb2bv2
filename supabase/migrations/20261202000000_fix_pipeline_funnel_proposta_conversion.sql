-- SP-0 fix #6: get_analytics_pipeline_metrics — proposta vira etapa real do funil.
-- Antes: 'Reunião → Proposta' usava attended/attended (100% trivial) e 'Proposta → Venda'
-- usava won/attended (etapa mal-rotulada). Idem nos conversion_trends mensais.
-- Fix: nova CTE proposta_leads/proposta_count + monthly_proposta; numerador/denominador corrigidos.
...
BEGIN
  ...
  -- Stage 3.5: Leads that reached pipe_propostas (any status — reached the proposta stage)
  proposta_leads AS (
  ...
  proposta_count AS (
    SELECT COUNT(*) AS cnt FROM proposta_leads
  ),
