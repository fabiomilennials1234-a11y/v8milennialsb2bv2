-- 20271008000000_leitores_saem_dos_espelhos.sql
--
-- SCRUM-639, janela de retirada dos leitores. Corpos capturados de PROD em
-- 2026-09-04 após a SCRUM-673. Cada função é CREATE OR REPLACE: assinatura,
-- ACL, SECURITY e search_path permanecem. A troca de pipe_* usa
-- negocio_projetado com funil_sistema e stage_key AS status; a projeção copia
-- exatamente os casts das views. Receita continua com a semântica atual nesta
-- janela; o caderno sale_events segue governando as funções já migradas.
--
-- Funções de manutenção removem deletes redundantes pelas views depois do
-- delete canônico em pipeline_entries. Os dois adaptadores de INSERT mantêm o
-- RETURNING legado, agora montado diretamente das tabelas canônicas.
--
-- Hashes MD5 de pg_get_functiondef antes da troca:
--   bulk_delete_leads(uuid[]): 8daac8fa31ef5aff9b320551eff5f338
--   custom_pipeline_stages_insert_fn(): 1889a911d60cb8ab47e645e02674793a
--   custom_pipelines_insert_fn(): bb91fd678fd29a3786731cf0f554ac5a
--   get_analytics_commercial_metrics(uuid,date,date,uuid,text): cd7ec4c38808a36a0193688e103240f2
--   get_analytics_financial_metrics(uuid,date,date,uuid,text): 2bd0b1e85a4738bd619b33dca4bc3dbe
--   get_analytics_overview_metrics(uuid,date,date,uuid,text): 6e70f02ba65670ab6688732f0af9fdf9
--   get_analytics_pipeline_metrics(uuid,date,date,text,uuid,uuid): a612ba26f82f9b5e09070f6959cf0db0
--   get_analytics_utm_metrics(uuid,date,date,uuid,text,text,text,text): d428c3fd36eed8f61f53c40348921f6b
--   get_leads_by_uf(character,integer,uuid): a5b95037cbf43e7f2edfd79192ed130a
--   get_mkt_origin_metrics(uuid,date,date,uuid): da117a5025197564159f8aef6eb33e3c
--   get_next_best_actions(integer,uuid): 952fba5d3212ee400e88e4af6af82293
--   get_ranking_data(integer,integer,uuid): 414d74e68df82d8e0083ebfa3c1eddda
--   get_uf_heatmap(uuid): 8694c0378c8676e98a354523fa75c8ad
--   purge_lead(uuid): b6588ddd3925d3759aeca6655d54f741
--   remove_demo_data(uuid): 9baa0df3c9217eed0e786465f0f17c0a
--   trigger_google_calendar_sync(): 8214f2d0207865b55a1e7295b2dce9af

BEGIN;

-- Falha fechada se outra migration mudou qualquer corpo depois da captura.
DO $preflight$
DECLARE
  r record;
  v_oid oid;
...
BEGIN
  ...
  LOOP
    v_oid := to_regprocedure('public.' || r.signature)::oid;
    IF v_oid IS NULL THEN
      RAISE EXCEPTION 'SCRUM-639 preflight: funcao ausente: %', r.signature;
    END IF;

    SELECT md5(pg_get_functiondef(v_oid)) INTO v_actual;
    IF v_actual IS DISTINCT FROM r.definition_md5 THEN
      RAISE EXCEPTION
        'SCRUM-639 preflight: corpo mudou para % (esperado %, atual %). Recapture e revise o diff.',
        r.signature, r.definition_md5, v_actual;
    END IF;
  END LOOP;

  IF to_regclass('public.negocio_projetado') IS NULL
     OR to_regclass('public.pipeline_entries') IS NULL
     OR to_regclass('public.pipeline_stages') IS NULL
     OR to_regclass('public.pipelines') IS NULL
     OR to_regclass('public.sale_events') IS NULL THEN
    RAISE EXCEPTION 'SCRUM-639 preflight: fonte canonica ausente';
  END IF;
END
$preflight$;
...
-- custom_pipeline_stages_insert_fn()
...
-- get_analytics_commercial_metrics(uuid,date,date,uuid,text)
...
BEGIN
  ...
  member_leads AS (
    ...
    FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'whatsapp') pw
  ...
  member_proposals AS (
    ...
    FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp
  ...
  origin_quality AS (
    ...
    FROM leads l
    LEFT JOIN (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp ON pp.lead_id = l.id AND pp.organization_id = p_org_id
...
-- get_analytics_financial_metrics(uuid,date,date,uuid,text)
...
BEGIN
  ...
  mrr_evolution AS (
    ...
    CROSS JOIN (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp
  ...
  cac_origin AS (
    SELECT
      ...
      END AS cac_estimate
    FROM leads l
    LEFT JOIN (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp
      ON pp.lead_id = l.id
  ...
  ticket_evolution_raw AS (
    ...
    JOIN (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp
...
-- get_analytics_overview_metrics(uuid,date,date,uuid,text)
...
BEGIN
  ...
  all_customers AS (
    ...
    FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas')
  ...
  churned_leads AS (
    ...
    FROM churn_base cb
    WHERE NOT EXISTS (
      SELECT 1 FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp2
  ...
  complete_journeys AS (
    SELECT
      ...
      (SELECT pw.created_at FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'whatsapp') pw
...
-- get_analytics_pipeline_metrics(uuid,date,date,text,uuid,uuid)
...
BEGIN
  ...
  leads_created AS (
    ...
    WHERE l.organization_id = p_org_id
      ...
      AND (p_member_id IS NULL OR EXISTS (
        SELECT 1 FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'whatsapp') pw
  ...
  proposta_leads AS (SELECT DISTINCT pp.lead_id FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp JOIN leads_created lc ON lc.lead_id = pp.lead_id WHERE pp.organization_id = p_org_id AND (p_member_id IS NULL OR pp.closer_id = p_member_id OR pp.responsible_id = p_member_id)),
  ...
  avg_days_confirmacao AS (
    ...
    FROM meeting_events me
    JOIN (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'whatsapp') pw ON pw.lead_id = me.lead_id AND pw.organization_id = p_org_id
    JOIN leads_created lc ON lc.lead_id = me.lead_id
    WHERE me.organization_id = p_org_id
      AND me.event_type = 'meeting_booked'
  ),
  ...
  monthly_qualified AS (
    ...
    FROM months_series m
    LEFT JOIN (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'whatsapp') pw ON pw.organization_id = p_org_id
  ...
  monthly_proposta AS (
    ...
    FROM months_series m
    LEFT JOIN (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp ON pp.organization_id = p_org_id
  ...
  IF p_level = 'leads' THEN
    ...
    FROM (
      SELECT
        ...
        l.created_at,
        COALESCE(
          (SELECT pw.status FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'whatsapp') pw WHERE pw.lead_id = l.id ORDER BY pw.created_at DESC LIMIT 1),
          'sem pipe'
        ) AS pipe_status,
        COALESCE(
...
-- get_leads_by_uf(character,integer,uuid)
...
BEGIN
  ...
  SELECT l.id, l.name, l.company, l.phone, l.uf_source,
    EXISTS (SELECT 1 FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp WHERE pp.lead_id = l.id AND pp.status = 'vendido'),
    COALESCE((SELECT SUM(pp.sale_value) FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp WHERE pp.lead_id = l.id AND pp.status = 'vendido'), 0)::numeric,
    l.created_at
  FROM leads l
...
-- get_mkt_origin_metrics(uuid,date,date,uuid)
...
BEGIN
  ...
  period_confirmacoes AS (
    ...
    FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'confirmacao') pc
...
-- get_next_best_actions(integer,uuid)
...
BEGIN
  ...
  stale_proposals AS (
    SELECT md5('pp:'||pp.id::text)::uuid, pp.lead_id, l.name, pp.id,
      'send_proposal'::text, ('Proposta parada há '||EXTRACT(DAY FROM now()-pp.stage_changed_at)::int||' dias')::text,
      ('Sem movimento desde '||to_char(pp.stage_changed_at,'DD/MM')||CASE WHEN COALESCE(pp.sale_value,0)>0 THEN ' — R$ '||to_char(pp.sale_value,'FM999G999G999')||' em jogo.' ELSE ' — retome o contato hoje.' END)::text,
      7, NULL::timestamptz, jsonb_build_object('derived',true,'source','negocio_projetado')
    FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp LEFT JOIN leads l ON l.id = pp.lead_id
    WHERE pp.organization_id = v_org_id AND pp.status NOT IN ('vendido','perdido') AND pp.stage_changed_at < now() - interval '7 days'
    ORDER BY COALESCE(pp.sale_value,0) DESC, pp.stage_changed_at ASC LIMIT 5
  ),
  missed_meetings AS (
  ...
  hot_idle_leads AS (
    SELECT md5('hl:'||l.id::text)::uuid, l.id, l.name, NULL::uuid,
      ...
      ('Qualificação alta e sem movimento desde '||to_char(COALESCE(t.last_activity_at,l.created_at),'DD/MM')||' — um contato hoje mantém o lead vivo.')::text,
      5, NULL::timestamptz, jsonb_build_object('derived',true,'source','ultimo_toque')
    FROM leads l
    CROSS JOIN LATERAL (
      SELECT public._metric_ultimo_toque(v_org_id, l.id, NULL::timestamptz) AS last_activity_at
    ) t
    WHERE l.organization_id = v_org_id AND l.deleted_at IS NULL
      AND (l.rating >= 4 OR l.qualification_score >= 70)
      AND COALESCE(t.last_activity_at,l.created_at) < now() - interval '48 hours'
      AND NOT EXISTS (SELECT 1 FROM follow_ups fu WHERE fu.lead_id = l.id AND fu.completed_at IS NULL AND fu.archived_at IS NULL)
      AND NOT EXISTS (SELECT 1 FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp WHERE pp.lead_id = l.id AND pp.status = 'vendido')
    ORDER BY l.qualification_score DESC NULLS LAST, l.rating DESC NULLS LAST, COALESCE(t.last_activity_at,l.created_at) ASC
    LIMIT 4
  )
  SELECT * FROM (
    SELECT * FROM stored
...
-- get_ranking_data(integer,integer,uuid)
...
BEGIN
  ...
  WITH sales_agg AS (
    SELECT se.sale_responsible_id AS member_id,
           SUM(COALESCE(se.sale_value, 0))::numeric AS total_value,
           COUNT(*)::int AS conversions
    FROM public.sale_events se
    WHERE se.organization_id = v_org_id
      AND se.event_type = 'sale'
      AND se.sale_responsible_id IS NOT NULL
      AND se.sold_at >= v_start_ts
      AND se.sold_at <= v_end_ts
      AND NOT EXISTS (
        SELECT 1
        FROM public.sale_events reversal
        WHERE reversal.event_type = 'sale_reversed'
          AND reversal.reversed_event_id = se.id
      )
    GROUP BY se.sale_responsible_id
  ),
  sales_data AS (
...
BEGIN
  ...
  SELECT l.uf, count(*)::bigint,
    count(*) FILTER (WHERE EXISTS (SELECT 1 FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp WHERE pp.lead_id = l.id AND pp.status = 'vendido'))::bigint,
    COALESCE(SUM((SELECT SUM(pp.sale_value) FROM (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'propostas') pp WHERE pp.lead_id = l.id AND pp.status = 'vendido')), 0)::numeric,
    v_unmapped
  FROM leads l
...
begin
  ...
  -- Reunião pertence ao pré-vendedor canônico; uma única chave evita atribuição ambígua.
  v_assigned_to := NEW.pre_sale_responsible_id;
  ...
  from (SELECT np.*, np.stage_key AS status FROM public.negocio_projetado np WHERE np.funil_sistema = 'confirmacao')
...
-- Prova estrutural global: nenhum corpo SQL/plpgsql continua usando os espelhos.
DO $postflight$
DECLARE
  v_offenders text;
BEGIN
  ...
  SELECT string_agg(
    DISTINCT functions.signature || ' -> ' || mirrors.name,
    E'\n' ORDER BY functions.signature || ' -> ' || mirrors.name
  )
  INTO v_offenders
  FROM functions
  JOIN mirrors
    ON functions.definition ~* ('(from|join)\s+(public\.)?' || mirrors.name || '\M')
    OR functions.definition ~* ('(insert\s+into|update|delete\s+from)\s+(public\.)?' || mirrors.name || '\M');

  IF v_offenders IS NOT NULL THEN
    RAISE EXCEPTION E'SCRUM-639 postflight: leitores/escritores legados restantes:\n%', v_offenders;
  END IF;
END
$postflight$;
