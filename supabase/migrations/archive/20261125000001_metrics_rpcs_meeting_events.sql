-- ADR-0007 phase 2: dashboard + ranking RPCs read meeting_events instead of
-- mutable pipe_confirmacao state. Attribution converges on canonical fields:
-- meetings → pre_sale_responsible_id (event snapshot); sales → sale_responsible_id
-- with legacy closer_id/responsible_id as fallback filters during transition.
-- Ranking payload gains additive fields: meetingsBooked, goalBooked, goalBookedProgress.
...
BEGIN
  ...
  -- 1. Leads captados (filtro de pessoa inclui campos canônicos + legados)
  ...
  -- 2. Reuniões — event-sourced (ADR-0007)
  -- Marcadas: eventos meeting_booked no período da marcação
  ...
  -- Comparecidas (realizadas): eventos meeting_held no período da reunião
  ...
  -- No-show: marcadas do período cuja data já passou e sem evento de realização
  ...
  -- 3. Propostas (filtro converge pra sale_responsible_id + legados)
  ...
  -- 5. Tempo médio de resposta: lead criado → primeira reunião marcada (evento)
