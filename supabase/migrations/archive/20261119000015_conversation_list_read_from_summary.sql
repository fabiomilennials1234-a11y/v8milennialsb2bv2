-- 20261119000015_conversation_list_read_from_summary.sql
--
-- V3 Fase 2: get_whatsapp_conversation_list passa a LER da tabela-resumo
-- whatsapp_conversation_summary (20261119000014) em vez do skip-scan recursivo.
--
-- Motivo: o skip-scan caminhava TODOS os telefones p/ ordenar por recência →
-- ~2.5s na org grande (500 conversas). A tabela-resumo (1 linha/conversa, mantida
-- por trigger) torna a lista um Index Scan no idx_wa_conv_summary_recency:
-- medido em prod ~34ms p/ 500 conversas (~85x vs 2906ms do fetch de 8000 linhas).
--
-- Unread: CTE única sobre idx_whatsapp_msgs_unread_cover (~13ms), COALESCE
-- last_read_at→now()-7d (read_state ainda vazio até wirar mark_conversation_read).
-- Cap p_limit 1000. SECURITY DEFINER enforça org via get_my_organization_ids().

CREATE OR REPLACE FUNCTION public.get_whatsapp_conversation_list(
  p_org uuid, p_instance uuid, p_limit integer DEFAULT 50, p_before timestamptz DEFAULT NULL
)
RETURNS TABLE (
  phone_number text, normalized_phone text, push_name text, last_message text,
  last_message_time timestamptz, last_message_direction text, last_message_sent_source text,
  lead_id uuid, is_group boolean, conversation_id uuid, archived_at timestamptz, unread_count integer
)
