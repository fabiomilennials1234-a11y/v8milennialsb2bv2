-- Fix: get_dashboard_metrics não retornava `taxaConversao` desde o rewrite
-- event-sourced (ADR-0007), então o card "Conversão" mostrava 0% para TODAS as
-- orgs (o frontend lê `d?.taxaConversao ?? 0`). Reintroduz a chave, calculada
-- como vendas/leads do funil (mesma base que o funil exibe "conv. total").
...
BEGIN
  ...
  -- Taxa de conversão do funil: vendas / leads captados (mesma base do funil)
