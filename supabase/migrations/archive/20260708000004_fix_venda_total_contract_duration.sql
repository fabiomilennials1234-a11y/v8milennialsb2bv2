-- ============================================================
-- FIX: Venda Total deve considerar contract_duration para MRR
--
-- Regra de negócio:
--   MRR:     v_venda_mrr   += sale_value          (valor mensal recorrente)
--            v_venda_total += sale_value * duration (valor total contratado)
--   Projeto: v_venda_proj  += sale_value
--            v_venda_total += sale_value           (valor pontual)
--   Unitário:               v_venda_total += sale_value
--
-- contract_duration nulo/zero → fallback de 1 mês.
-- ticketMedioMRR continua refletindo o valor mensal recorrente médio (não o total).
-- ============================================================
...
BEGIN
  ...
  -------------------------------------------------------------------
  -- 3. Propostas / Vendas — com items e contract_duration
  -------------------------------------------------------------------
  ...
  -- contract_duration nulo/zero → fallback de 1 mês via GREATEST
  FOR rec IN
    SELECT
      pp.id                                              AS proposta_id,
      pp.sale_value                                      AS prop_sale_value,
      pp.product_type                                    AS prop_product_type,
      GREATEST(1, COALESCE(pp.contract_duration, 1))     AS prop_contract_duration,
      COALESCE(
  ...
  LOOP
    IF jsonb_array_length(rec.items) > 0 THEN
      DECLARE
        ...
        prop_mrr     NUMERIC := 0;
        prop_proj    NUMERIC := 0;
        contract_dur INT     := rec.prop_contract_duration;
      BEGIN
        ...
        LOOP
          ...
          IF item_type = 'mrr' THEN
            ...
            prop_total := prop_total + item_val * contract_dur; -- MRR × duração
          ELSIF item_type = 'projeto' THEN
            prop_proj  := prop_proj  + item_val;
            prop_total := prop_total + item_val;                -- Projeto: pontual
          ELSE
            prop_total := prop_total + item_val;                -- Unitário: pontual
          END IF;
        END LOOP;
      ...
      END;
    ELSE
      -- Fallback: usar sale_value e product_type da proposta diretamente
      DECLARE
  ...
  -- Tickets médios
  -- ticketMedio    = valor médio total contratado por venda (inclui duração do MRR)
  -- ticketMedioMRR = valor médio mensal recorrente (não multiplica pela duração)
