-- 20270211000005_gestor_rls_sweep_helper_union.sql
-- Sweep (ADR-0021): ~113 policies SELECT/ALL trocam a subquery INLINE em
-- team_members pelos helpers SECURITY DEFINER get_my_organization_ids /
-- get_my_admin_organization_ids / get_my_team_member_ids — que carregam a UNIÃO
-- das orgs do Gestor de Portfólio. Sem isso o gestor lia os vínculos mas nenhuma
-- linha dessas tabelas (conversations, campanhas, workflows, products, whatsapp,
-- dashboards, quotes, deals, ...). Também paga a dívida anti-recursão Realtime
-- (CLAUDE.md: nunca SELECT ... FROM team_members inline em policy).
--
-- Equivalência p/ NÃO-gestor provada: get_my_organization_ids() ≡ orgs de
-- team_member ativas do usuário; get_my_gestor_organization_ids() = vazio p/
-- não-gestor. Logo TODO rewrite é no-op para usuários existentes e só abre gestor.
--
-- Método: colapsa whitespace + replace EXATO de string (sem regex → sem risco de
-- parênteses). Bloco DO atômico e AUTO-VALIDANTE: se qualquer policy não limpar
-- 100% ou desbalancear parênteses → RAISE → rollback total (fail-closed).
-- Validado por dry-run em prod (113/115 auto-limpas). Os 5 restantes (2 correlated
-- EXISTS + 3 por-responsabilidade) são recriados explicitamente abaixo, com a
-- união do gestor.

-- ── Parte 1: bulk (exact-string replace + validação) ────────────────────────
DO $sweep$
DECLARE
...
-- ── Parte 2: correlated EXISTS (reescritos p/ helper) ───────────────────────
...
-- ── Parte 3: por-responsabilidade (org via helper + branch gestor = vê tudo) ─
-- Gestor de Portfólio vê TODA a org vinculada (como admin), sem alargar o acesso
-- de membro (a cláusula de responsabilidade só ganha um OR gestor, vazio p/ resto).
