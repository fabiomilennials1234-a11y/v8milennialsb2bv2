-- Add explicit Source, Medium, Content and Term access without broadening grants.
-- Includes prior definitions for recovery with retained eleven-field history.
BEGIN;
...
ALTER TABLE public.workflow_data_grants ADD CONSTRAINT workflow_data_grants_fields_check CHECK (
  fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.tags']::text[] AND array_position(fields, NULL) IS NULL AND cardinality(fields) <= 11
);
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.tags']::text[])
    OR array_position(p_fields, NULL) IS NOT NULL OR cardinality(p_fields) > 11
...
BEGIN
  ...
  IF jsonb_typeof(p_definition) IS DISTINCT FROM 'object'
    ...
    OR NOT (p_required_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone','lead.qualification_score','lead.utm_campaign','lead.utm_source','lead.utm_medium','lead.utm_content','lead.utm_term','lead.tags']::text[]) THEN
