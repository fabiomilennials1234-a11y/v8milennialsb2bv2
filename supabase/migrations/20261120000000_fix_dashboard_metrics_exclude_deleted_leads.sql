-- 20261120000000_fix_dashboard_metrics_exclude_deleted_leads.sql
--
-- Fix: get_dashboard_metrics contava leads soft-deletados (deleted_at IS NOT NULL)
-- no totalLeads, porque o COUNT direto sobre `leads` não filtrava deleted_at.
-- Sintoma: dashboard mostrava N leads, kanban (pipeline_entries) mostrava N-1 cards.
-- Detectado na org Forte Sistemas (3 dashboard vs 2 cards): 1 lead de teste de QA
-- soft-deletado seguia contando na métrica.
--
-- Correção: adiciona `AND deleted_at IS NULL` nos 2 pontos que leem `leads`
-- diretamente — passo 1 (Leads captados) e passo 5 (Tempo médio de resposta).
-- Passo 5 já era imune na prática (filtra first_contact IS NOT NULL, e lead
-- deletado de teste não tem pipe_confirmacao), mas mantém consistência.
-- Demais passos leem pipe_* (views sobre pipeline_entries) — lead sem entry
-- não aparece, então não precisam do filtro.
--
-- CREATE OR REPLACE idempotente. Sem mudança de assinatura.
