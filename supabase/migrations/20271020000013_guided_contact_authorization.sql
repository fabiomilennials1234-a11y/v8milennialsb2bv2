-- Expand contact fields without modifying existing approvals or published versions.
-- This forward file includes and supersedes the function/constraint definitions of 12.
BEGIN;
...
ALTER TABLE public.workflow_data_grants ADD CONSTRAINT workflow_data_grants_fields_check CHECK (
  fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone']::text[] AND array_position(fields, NULL) IS NULL AND cardinality(fields) <= 4
);
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'access_denied' USING ERRCODE = '42501';
  END IF;
  IF p_fields IS NULL OR NOT (p_fields <@ ARRAY['lead.name','lead.company','lead.email','lead.phone']::text[])
    OR array_position(p_fields, NULL) IS NOT NULL OR cardinality(p_fields) > 4
