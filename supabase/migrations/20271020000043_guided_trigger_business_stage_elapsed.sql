-- Measure current-stage elapsed time from the trusted clock of the exact trigger business.
BEGIN;
...
BEGIN
  ...
  IF p_entry_id IS NULL OR p_fields IS NULL OR cardinality(p_fields) = 0 OR cardinality(p_fields) > 3
    OR array_position(p_fields, NULL) IS NOT NULL
    OR NOT (p_fields <@ ARRAY['business.trigger.stage','business.trigger.value','business.trigger.stage_elapsed']::text[])
  ...
  RETURN jsonb_build_object(
    'entry', jsonb_build_object('id', v_entry.id, 'pipeline_id', v_entry.pipeline_id,
      ...
      'stage_elapsed_seconds', CASE WHEN 'business.trigger.stage_elapsed' = ANY(p_fields) AND v_entry.stage_changed_at IS NOT NULL
        THEN extract(epoch FROM statement_timestamp() - v_entry.stage_changed_at)::double precision ELSE NULL END),
...
CREATE OR REPLACE FUNCTION public.validate_guided_trigger_business_stage_elapsed_version()
...
BEGIN
  WITH RECURSIVE conditions(node_id, condition) AS (
    ...
    SELECT conditions.node_id, child FROM conditions CROSS JOIN LATERAL jsonb_array_elements(
      ...
      ((condition ->> 'operator') = ANY(ARRAY['equals','not_equals','greater_than','greater_than_or_equal','less_than','less_than_or_equal'])) IS DISTINCT FROM true
      OR jsonb_typeof(condition -> 'value') IS DISTINCT FROM 'number'
      OR CASE WHEN jsonb_typeof(condition -> 'value') = 'number' THEN (condition ->> 'value')::numeric < 0 ELSE false END
      OR ((condition ->> 'unit') = ANY(ARRAY['minutes','hours','days'])) IS DISTINCT FROM true)
    INTO v_count, v_invalid_node_ids FROM conditions WHERE condition ->> 'field' = 'business.trigger.stage_elapsed';
  IF v_count = 0 THEN RETURN NEW; END IF;
  IF NOT ('business.trigger.stage_elapsed' = ANY(NEW.required_fields)) THEN
...
END;
$$;
REVOKE ALL ON FUNCTION public.validate_guided_trigger_business_stage_elapsed_version() FROM PUBLIC, anon, authenticated, service_role;
CREATE TRIGGER validate_guided_trigger_business_stage_elapsed_version
BEFORE INSERT OR UPDATE OF definition, required_fields ON public.workflow_guided_versions
FOR EACH ROW EXECUTE FUNCTION public.validate_guided_trigger_business_stage_elapsed_version();
