-- INVOKER precisa consultar a visibilidade via RLS; o helper interno
-- can_link_or_read_lead não concede EXECUTE a authenticated em produção.
-- Preserva RLS e não amplia os grants do helper nem usa DEFINER.
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.leads l WHERE l.id=v_entry.lead_id AND l.organization_id=v_entry.organization_id AND l.deleted_at IS NULL) THEN
