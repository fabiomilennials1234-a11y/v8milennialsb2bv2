-- Atender chamada de entrada exige id de rede (Fase 1 do contrato TorqueCalls).
-- ROLLBACK pareado: rollback/20270730000008_voip_reserve_inbound_requires_tc_call_id.sql
--
-- O DEFEITO
-- ---------
-- No ramo de entrada (`p_existing_call_id IS NOT NULL`), a reserva fazia o
-- UPDATE que grava `operator_user_id` e SÓ DEPOIS devolvia `tc_call_id: null`
-- — porque a linha do ringing pode não ter id de rede ainda. O CRM então nega
-- corretamente em call-plane.ts (`reserve_sem_tc_call_id` → `reserve_failed`),
-- mas o estrago já está feito: a linha ficou com o operador gravado e status
-- `ringing`, ou seja, DENTRO do conjunto do UNIQUE parcial
-- `idx_voip_calls_one_live_per_operator`. O fail-closed do CRM protege a
-- assinatura e trava o vendedor: mesmo defeito do C1, por outra porta.
--
-- A CORREÇÃO
-- ----------
-- `AND tc_call_id IS NOT NULL` no WHERE do UPDATE do ramo de entrada. A linha
-- sem id de rede simplesmente não casa, `v_call_id` volta NULL, e a função
-- devolve `call_not_answerable` SEM ter escrito nada. A negativa passa a ser
-- pura — que é o que "fail-closed" tem que significar: nega e não deixa rastro
-- que atrapalhe a próxima tentativa.
--
-- Deliberadamente NÃO se acrescenta CHECK de formato na coluna: o CTO avaliou e
-- deixou fora desta fatia. Aqui a exigência é só de EXISTÊNCIA.
--
-- Corpo partindo de 20270730000006 (a versão vigente), não de 20270730000003
-- nem da fundação — copiar de qualquer uma das duas apagaria em silêncio a
-- cunhagem do tc_call_id na reserva de saída.
--
...
BEGIN
  ...
  BEGIN
    IF p_existing_call_id IS NOT NULL THEN
      ...
      --
      -- `tc_call_id IS NOT NULL` faz parte do PREDICADO, não de uma checagem
      -- depois. Sem id de rede não há o que assinar — o cid tem que ser a mesma
      -- string que vai no path, senão a VPS recusa — e o chamador vai negar de
      -- qualquer jeito. Se a checagem viesse depois do UPDATE, a negativa
      -- deixaria a linha com `operator_user_id` gravado em status `ringing`,
      -- dentro do UNIQUE parcial `idx_voip_calls_one_live_per_operator`: o
      -- operador ficaria consumido por uma chamada que ele nunca atendeu, e a
      -- próxima tentativa dele devolveria `operator_busy`. Negar aqui é negar
