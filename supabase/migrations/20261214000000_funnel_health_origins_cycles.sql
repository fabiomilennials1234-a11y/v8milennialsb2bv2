-- Saúde do Funil — filtro multi-origem + métricas de ciclo de venda.
--
-- 1. get_funnel_health ganha p_origins text[] (NULL/vazio = todas as origens)
--    e passa a devolver "cycles": tempo médio em dias da entrada do lead à
--    venda (lead_to_sale_days), da reunião realizada à venda
--    (meeting_to_sale_days) e da entrada à reunião (lead_to_meeting_days).
--    Venda = primeira entry 'vendido' no pipe Orçamentos; data da venda =
--    COALESCE(closed_at, stage_changed_at). Reunião = primeiro meeting_held
--    (occurred_at, ADR-0007).
-- 2. get_funnel_health_stage_leads ganha o mesmo p_origins.
--
-- Assinaturas antigas são DROPadas (não sobrecarregadas) para evitar
-- ambiguidade PGRST203 no PostgREST.

DROP FUNCTION IF EXISTS public.get_funnel_health(uuid, timestamptz, timestamptz);
...
REVOKE ALL ON FUNCTION public.get_funnel_health(uuid, timestamptz, timestamptz, text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_funnel_health(uuid, timestamptz, timestamptz, text[]) TO authenticated, service_role;

DROP FUNCTION IF EXISTS public.get_funnel_health_stage_leads(uuid, timestamptz, timestamptz, text);

CREATE OR REPLACE FUNCTION public.get_funnel_health_stage_leads(
  p_org_id uuid,
  p_start_date timestamptz,
  p_end_date timestamptz,
  p_stage text,
...
REVOKE ALL ON FUNCTION public.get_funnel_health_stage_leads(uuid, timestamptz, timestamptz, text, text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_funnel_health_stage_leads(uuid, timestamptz, timestamptz, text, text[]) TO authenticated, service_role;
