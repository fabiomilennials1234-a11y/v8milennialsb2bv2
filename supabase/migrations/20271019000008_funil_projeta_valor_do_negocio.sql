-- O modal de negócio grava o valor canônico em `deals.value`. O kanban lê
-- `get_pipeline_page`, que ainda devolvia somente `pipeline_entries.metadata`;
-- por isso o modal mostrava o número salvo enquanto o card seguia em
-- "Sem valor". A função passa a projetar `deals.value` sobre a chave
-- `metadata.sale_value` do payload, sem reintroduzir uma segunda escrita.
...
BEGIN
  ...
  SELECT pe.id, pe.pipeline_id, pe.lead_id, pe.stage_key, pe.assigned_to, pe.notes,
    CASE
      WHEN d.value IS NULL THEN pe.metadata
...
-- Guardas estruturais do contrato: o leitor continua tenant-scoped, preserva
-- o retorno `metadata` e não duplica a entry quando encontra o negócio.
DO $$
DECLARE v_def text;
BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_def
    FROM pg_proc p
   WHERE p.oid = 'public.get_pipeline_page(text,text,uuid,integer,timestamptz,text,uuid,uuid[],text[],integer,integer,integer,integer,text,text,timestamptz,timestamptz,timestamptz,timestamptz,text[],timestamptz,text[],text[],boolean,text[],text[],integer,integer,uuid)'::regprocedure;

  IF v_def NOT LIKE '%d.value%' OR v_def NOT LIKE '%jsonb_set%' THEN
    RAISE EXCEPTION 'get_pipeline_page nao projeta deals.value no metadata';
  END IF;
