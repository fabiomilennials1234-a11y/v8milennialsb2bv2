...
async function withTimeout<T>(p: Promise<T>, ms: number): Promise<T> {
  // A timeout does not cancel database work. Track the original promise so a
  // standalone graceful shutdown (or Edge isolate) cannot abandon it silently.
  ...
  try {
    return await Promise.race([
      ...
      new Promise<T>((_, reject) => {
        timer = setTimeout(() => reject(new Error("processing_timeout")), ms);
      }),
    ]);
  } finally {
...
export interface WhatsAppWebhookOptions {
  /** Standalone rollout gate; evaluates the database-resolved instance, never a URL hint. */
  allowInstance?: (instanceId: string, organizationId: string) => boolean;
  /** Dedicated ingress must not ACK receipts before their target exists. */
...
export function createWhatsAppWebhookHandler(options: WhatsAppWebhookOptions = {}) {
  return withErrorBoundary("whatsapp-webhook", async (req: Request) => {
    ...
    const rl = options.trustedQueuedReplay ? { allowed: true } : checkRateLimit(sourceIp);
    if (!rl.allowed) {
      await logRuntime({
        module: "webhook",
    ...
    if (!options.trustedQueuedReplay && typeof payload.timestamp === "number") {
    ...
    const persistentRl = options.trustedQueuedReplay ? { allowed: true, remaining: 0 } : await checkRateLimitPersistent(
      supabase,
    ...
    if (options.allowInstance && !options.allowInstance(instance.id, instance.organization_id)) {
