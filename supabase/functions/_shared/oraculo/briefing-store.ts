...
export function createBriefingStore(db: SupabaseClient) {
  return {
    async current(actor: OracleActor): Promise<unknown | null> {
      const { data, error } = await db.rpc(
        actor.isAdmin ? "oraculo_admin_briefing_current" : "oraculo_member_briefing_current",
        {
    ...
    async open(actor: OracleActor, briefingId: string): Promise<unknown> {
      const { data, error } = await db.rpc(
        actor.isAdmin ? "oraculo_open_admin_briefing" : "oraculo_open_member_briefing",
        {
