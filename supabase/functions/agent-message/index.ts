...
Deno.serve(withErrorBoundary('agent-message', async (req) => {
  ...
  try {
    ...
    const { from, channel, organization_id, push_name, incoming_message_type, instance_id, message_context } = body; // from = phone number ou user_id
    ...
    if (await hasActiveWorkflowsForTrigger(supabase, organization_id, "lead_replied")) {
      ...
      if (repliedLead) {
        ...
        await fireTrigger({
          ...
          context: { trigger: "lead_replied", channel, message, instance_id, message_context },
          source: "copilot",
        }).catch((err) => {
