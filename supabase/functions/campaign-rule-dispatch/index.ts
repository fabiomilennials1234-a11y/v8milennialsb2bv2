      ...
      try {
        ...
        if (sendResult.success) {
          ...
          await supabase.rpc("increment_whatsapp_rate_limit", {
            p_organization_id: campanha.organization_id,
            p_instance_id: instance.id,
          }).catch((e) => console.warn("[campaign-rule-dispatch] increment_whatsapp_rate_limit failed:", e));
          ...
          }).catch((err) => console.warn("[campaign-rule-dispatch] lead_history insert failed:", err));
        ...
        console.error("[campaign-rule-dispatch] Row error:", row.id, rowError);
        await markFailed(supabase, row.id, rowError instanceof Error ? rowError.message : String(rowError)).catch(() => {});
        failed++;
      }
    }
