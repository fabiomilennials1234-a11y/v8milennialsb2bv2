...
- **UF (Estado)**: The Brazilian state a Lead belongs to. Stored as the 2-letter UF code (one of the 27 valid UFs), displayed as "Estado" in the UI. The third **standard question** alongside `segment` and `faturamento` (flat Lead fields fillable by webhook, manual dropdown, or AI extraction). Can be **derived automatically from the phone DDD** (area code → UF) as a low-confidence fallback; a manually answered Estado always wins over the DDD-derived one and is never overwritten by it. Surfaced in the **Mapa** tab of the Central de Comando (whole-base choropleth, independent of the month selector).
