...
## Rodada 14 — pré-requisitos de IA e gates do Copilot (2026-09-11)

Status da TorqueSDR HTTP200/connected, campo openai_apikey presente e vazio. OpenAPI oficial 2.1.1 exige chave OpenAI válida para /message/download com transcrição. Corrige interpretação anterior: ausência de texto está comprovada; bug interno do fornecedor não. Chave solicitada por configuração segura, sem segredos no repositório. QA sem OPENROUTER_API_KEY e zero agentes ativos.

agent-message validava OPENROUTER_API_KEY antes de todos os gates e retornava500 mesmo sem agente ativo. Credencial/client agora inicializados apenas antes do engine. Preserva autenticação exata, tenant obrigatório, workflow lead_replied e criação opt-in de lead. Testes do handler: sete cenários; suítes batch/gate totalizam23 testes aprovados. Corrigido mock logEvent ausente na suíte batch. Deno:31 diagnósticos atuais, mesmos31 no HEAD anterior; aprovação integral não alegada.

Deploy agent-message somente QA concluído. Sondagem externa usando service_role retornada pela API de gerenciamento foi recusada401 pela guarda interna (não erro de gateway), tanto no caso sem agente quanto sem tenant; caso não autorizado também401. Não afrouxada autenticação. Homologação viva do salto interno e resposta real do Copilot continuam pendentes; testes isolados não substituem isso.
