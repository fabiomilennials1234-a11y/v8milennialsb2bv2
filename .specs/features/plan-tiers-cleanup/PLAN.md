...
- [x] **Step 4:** Deploy FEITO (2026-07-02, ordem explícita do CTO, fora do auto mode): 6 fns × DEV + 6 fns × **PROD** — 12/12 "Deployed Functions". Migrations 20270105000000+0001 APLICADAS EM PROD + registradas. Pré-deploy: auditoria de prod achou 5 orgs que quebrariam no gate fail-closed (Bennedita Pan + Bertin: copilot ativo em torque-2.0 sem turbo; Motor 100 + TrooveBR: chat em plano free; Teste a1) → overrides `organization_features` com override_reason rastreável (billing a revisar pelo CTO). Smoke pós-deploy: risco residual ZERO nos 3 gates; workflows normais (0 skipped_plan); copilot fluindo (9 msgs/15min); 0 plan-denied em runtime_logs. Grandfathering PROD: VitrineVET 18, Vanilla Brasil 10, Alamaster 7, Maria Bonita 6. HGE (2.0) perde oráculo (1 uso/30d) — spec-correto, sem override.
