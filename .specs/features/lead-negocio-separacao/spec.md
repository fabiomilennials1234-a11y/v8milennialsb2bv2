...
| **T5** | **Migrar os 39.613 cards existentes para negócios** | T2, T3, T4 + decisão do corte |
...
**A trava da recompra.** Duas travas no banco, não uma — uma delas fácil de esquecer.
Enquanto as duas existirem, "vários negócios por cliente" não existe. Removê-las é o
ponto de não-retorno da feature.
...
incidente com os comentários do lead. Pior: a regra de edição não valida o destino, então
seria possível editar um negócio e **empurrá-lo para outra empresa**. Precisa ser
...
| **B** | Cal.com | **Ingest nunca cria negócio**, mas a reunião ganha casa no lead | `leads` tem zero coluna de reunião: sem card, a data seria **descartada**, não só o lembrete. E a Milennials — org piloto — é a maior usuária do Cal.com (76 de 102 leads) |
...
- **São três cadeados, não dois.** Além dos dois em `pipeline_entries`, existe
  ...
  destrava nada.
- **São 52.588 cards, não 39.613.** Duas tabelas: `pipeline_entries` 36.412 (66 orgs) e
  `custom_pipe_entries` 16.176 (24 orgs).
...
O botão já está na tela **sem fazer nada**. Enquanto não for decidido, é botão de mentira.
...
("Reposição trimestral"), ou o sistema deriva sozinho? E na migração, os 52.588 herdariam
...
Hoje uma proposta marcada como vendida vira cliente de carteira. Com três negócios do
mesmo cliente, os três fecham em separado — e o cliente de carteira continua sendo um só.
Isso já está certo na estrutura, mas ninguém definiu o que a tela mostra quando um negócio
do cliente está ganho e outro está em negociação.
...
T2 ──┐
T3 ──┼── T5 (migração dos 39.613)
T4 ──┘
T6 ── inventário → limpeza → validação
T7 ── decisão de formato → fecha T1
```

**T3 tem prazo de validade.** Apagar a duplicidade de posição é seguro **hoje**, porque a
tabela está vazia. Essa janela fecha no instante em que a migração rodar.
