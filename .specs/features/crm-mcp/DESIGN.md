...
### C2 — PAT infra + cliente RLS-scoped per-user + tracer tool + gate de allowlist runtime ⭐ (BACKEND + UI ENTREGUES 2026-06-25)
O tracer-bullet vertical. Entrega ponta-a-ponta o caminho mais fino, **com todos os fixes HIGH embutidos** (não diferidos). **Backend** na branch `feat/crm-mcp/c2-pat-infra` (PR #889): migration `20270101000000` + `supabase/functions/crm-mcp/` + espinha estendida + tracer + unit/anchor (131 unit verdes, lint/fmt/type-check limpos). **UI** na branch `feat/crm-mcp/c2-ui-pat` (stacked em #889): `create-pat` edge fn (gera token→grava hash→retorna plaintext 1×, RLS do user, master-reject) + `PersonalAccessTokensPanel` (criar/listar/revogar, display-once com snippet de conexão MCP, aviso role-aware) na aba "Tokens IA" de Configuracoes; hooks `usePersonalAccessTokens`; build ✓, tsc 0 erros, 4 unit. **Resta na C2 (deploy):** aplicar migration em dev + secrets (`CRM_MCP_JWT_SIGNING_KEY`/`KID`, `CRM_MCP_PAT_PEPPER`, `CRM_MCP_TOKEN_ENV`) + confiar a chave pública ES256 no JWKS do projeto + canary round-trip no boot. ⚠️ Coerência de produto a decidir: `personal_access_tokens` (MCP, per-user) coexiste com `api_keys` (REST org-level) — avaliar unificação/posicionamento.
