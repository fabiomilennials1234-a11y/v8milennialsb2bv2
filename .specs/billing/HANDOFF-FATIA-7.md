...
9. **`p_manual_final_cents` é o preço MENSAL, não o total da cobrança.** Aviso já está no `COMMENT ON FUNCTION` das duas funções, e a renomeação (+ guarda que recusa override que AUMENTA o preço) está proposta na **issue #1559** como fatia própria. Custou duas asserções vermelhas: passei `charge_cents` de um ciclo anual e o motor devolveu `manual_discount_cents = 0` — não deu desconto, deu **aumento de 12x** (`monthly_cents` virou o total). A tela tem que enviar o mensal. O nome do parâmetro não diz isso, e o motor aceita calado.
