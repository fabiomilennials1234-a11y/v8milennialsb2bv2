...
- Remaining: deployed worker execution; date/select/boolean; cross-type lifecycle semantics; broader search/a11y/limits audit. Remote custom-name search currently uses ILIKE, so do not claim accent-insensitive remote matching from the static catalogue's normalization tests. The catalogue currently admits text and number definitions; date/select/boolean remain unavailable until implemented.
...
The selector retains compatible numeric comparisons when moving between the standard score and custom numeric definitions. Text-to-number changes clear incompatible values. Date/select/boolean, option lifecycle, complete cross-type repair, remote accent matching and cost/limit feedback remain open; this is not ticket 07 certification.
