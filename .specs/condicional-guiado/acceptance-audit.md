...
- Remaining: organizational per-definition grants/publication/execution; number/date/select/boolean; cross-type lifecycle semantics; broader search/a11y/limits audit. Remote custom-name search currently uses ILIKE, so do not claim accent-insensitive remote matching from the static catalogue's normalization tests. The catalogue admits only text definitions until other evaluators are implemented.
- The grant panel explicitly indicates personal-test-only availability for custom fields; this temporary gate must be removed when organizational authorization is implemented. It is not the final product scope. All tickets stay open.

## Organizational custom text scopes

Per-definition approval and atomic automatic reads are now implemented and real-tested, including mixed fields, exact projection, revocation, foreign identities and post-deletion revocability. This is not a custom publication/worker certificate: the finalizer and editor approval gate still require completion. A defensive 256-scope budget now exists in the grant validator and public evaluator; ticket 21 must validate its performance and corresponding editor/publication feedback.
