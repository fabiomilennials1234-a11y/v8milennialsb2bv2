...
- Remaining: deployed worker execution; number/date/select/boolean; cross-type lifecycle semantics; broader search/a11y/limits audit. Remote custom-name search currently uses ILIKE, so do not claim accent-insensitive remote matching from the static catalogue's normalization tests. The catalogue admits only text definitions until other evaluators are implemented.
