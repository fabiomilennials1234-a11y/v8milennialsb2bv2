...
- Remaining: deployed worker execution; date/select; cross-type lifecycle semantics; broader search/a11y/limits audit. Remote custom-name search currently uses ILIKE, so do not claim accent-insensitive remote matching from the static catalogue's normalization tests. The catalogue currently admits text, number and boolean definitions; date/select remain unavailable until implemented.
