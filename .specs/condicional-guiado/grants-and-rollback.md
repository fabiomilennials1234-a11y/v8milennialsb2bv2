...
### Lead profile fields migration 20271017000028

Applied and registered atomically on preview only. Adds independent lead.segment, lead.urgency and lead.faturamento approvals and exact projections in scalar and mixed readers. All three are free-text data, including revenue ranges; no numeric coercion or customer-data rewrite occurs. The full superset retains reference-presence publication validation and earlier readers. Effective privileges verified for all six replaced signatures: grant writer authenticated-only, readers/finalizer service-only, no anonymous EXECUTE, pinned search_path=public.

Rollback restores the prior function vocabulary and retains the expanded CHECK plus all records, approvals, versions and execution pins. New-field reads/publication fail closed until supported code/schema is restored; never narrow the CHECK over retained approvals. Twenty-nine-migration rehearsal passed with seventeen-field synthetic grant/version history and exact finalizer restoration. Full recovery: 00..11, 28, 14, 21, 23, 24. Fresh installation remains numerical. Personal-test/publication endpoints updated only on preview.

## Migration 29 — personal custom-field text evaluation

Adds only test_guided_condition_custom_fields(uuid,uuid,uuid[]), a STABLE SECURITY INVOKER reader with search_path=public. It validates a live readable lead and the complete organization-scoped UUID definition set before projecting current names, types and answers. Definitions without answers return null; missing definitions raise PT422. No customer records are rewritten.
...
## Migrations 39–41 — exact trigger-business stage

39 adds authenticated-only SECURITY INVOKER reader for personal tests. Caller RLS governs exact entry, pipeline and stage references. 40 admits only `business.trigger.stage` into scope vocabulary, adds service-only organization reader and publication trigger validating every nested pipeline/stage pair. Reader still requires matching workflow/organization grant and live exact entry. 41 marks locking reader VOLATILE; PostgreSQL rejected `FOR SHARE` under initial STABLE declaration during real red test.

Rollback 41 restores preceding volatility, 40 removes organization reader/publication trigger and restores prior scope helper, and 39 removes personal reader. No entry, pipeline, stage, grant, version or execution is deleted. The 42-migration transactional rollback/reapply rehearsal passed. Effective privileges: personal reader authenticated-only, organization reader service-only, publication trigger function directly callable by no API role. Preview endpoints only; worker and production undeployed.

## Migration 42 — exact trigger-business financial value
