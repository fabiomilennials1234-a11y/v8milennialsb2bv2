-- Templates editáveis do Estúdio. Preserva integralmente os painéis existentes.
-- A carga é feita UMA vez nesta migration e no INSERT de uma organização nova.
-- Abrir o Estúdio não semeia nada: excluir uma aba é definitivo, sem ressurreição.
-- Deploy: publicar os adaptadores antes de aplicar esta migration; retirar as
-- abas do Comando apenas com as quatro origens disponíveis no Estúdio.
-- RLS e ACL de metrics_studio_panels não mudam.
