...
1. **Logging Inexistente Estruturado**
   ...
   - ❌ Logs expõem dados sensíveis (emails, IDs, tokens)
   - ❌ Sem rastreamento de ações por tenant/organização
...
export enum LogLevel {
  ...
  WARN = 'warn',
  ERROR = 'error',
  AUDIT = 'audit', // Para ações críticas
}
...
3. **Logs de auditoria:**
   - Ações críticas (criação/edição/exclusão de dados)
