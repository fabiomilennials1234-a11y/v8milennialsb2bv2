...
**Seu output habilita o fan-out.** O orchestrador só paraleliza `revisor ‖ qa` se a sua seção `QA` reportar que **esta branch não introduziu** falha de lint/tipos/unit/build. A pré-condição é **delta, não zero absoluto** — use `lint:ratchet` e `typecheck:ratchet`, não `lint` cru. Reporte o estado real — nunca presuma verde, e nunca reporte dívida herdada como vermelho seu.
...
- [ ] `npm run lint:ratchet` passa — **use este, não `npm run lint`**
- [ ] `npm run typecheck:ratchet` passa
- [ ] `npm run lint:deps:check` passa (se mexeu em import cross-module)
- [ ] `npm run test:unit` passa
- [ ] `npm run test:integration` passa (se mudou backend/DB)
- [ ] `npm run build` passa

**Os gates são DELTA, não zero absoluto.** Este repo tem 805 erros de tipo de baseline e **29.142 warnings de eslint**. `npm run lint` sai 0 mas imprime 29k linhas terminadas em `✖ 29142 problems` — ler isso como vermelho e reportar vermelho **trava o fan-out por dívida que não é sua**. Os ratchets reprovam só o que **esta branch introduziu** e imprimem o herdado como informativo, com teto de 20 linhas.

**Não faça arqueologia de dívida alheia.** Se um gate reclama de algo que você não escreveu, o ratchet já classificou: `HERDADO` = ignore e siga. Gastar output provando que a falha é antiga é o desperdício que os ratchets existem pra matar.

Se `test:unit` ou `build` falhar, **teste o base antes de reportar**: `git stash && npm run test:unit`. Falha idêntica no base = herdada — reporte como tal e o fan-out segue. Só falha exclusiva da branch é sua.
...
## QA
<resultado REAL de lint:ratchet / typecheck:ratchet / test:unit / build — verde ou vermelho,
nunca presumido. Estes são a pré-condição DELTA do fan-out revisor ‖ qa: verde = a branch
não introduziu falha, mesmo que o repo tenha dívida herdada.
Se algo está vermelho, diga se é INTRODUZIDO por esta branch ou HERDADO (confirmado no base).
