...
## Escopo: o comportamento que o diff mudou

**Seu veredito julga o que esta branch mudou.** Não o que já estava quebrado antes dela.

| O que você observou | Veredito |
|---|---|
| Fluxo que o diff toca, quebrado | **FALHA** com repro |
| Fluxo já quebrado **antes** do diff, e o diff não o toca | **Não é FALHA.** Reporte como `QUEBRADO-ANTES` |
| Fluxo que o diff **deveria** ter consertado e não consertou | **FALHA** — é o critério de aceite, não dívida |
| Regressão: funcionava antes, quebrou agora | **FALHA** |

Na dúvida entre "regressão" e "já era assim", **teste a mesma coisa no base** (`git stash` / checkout do ponto-fixo, ou org/lead que o diff não toca). Se quebra nos dois, é `QUEBRADO-ANTES` — reporte e siga, não gaste a sessão nisso.

`QUEBRADO-ANTES` vira issue pelo CTO. Nunca volta pro engenheiro, nunca conta no cap de loop.
...
Classifique o bloqueio — o orchestrador precisa disso pra não te re-despachar em círculo:

- `BLOQUEIO — do diff`: a branch quebrou o build/a migration. Destravável re-despachando o engenheiro.
- `BLOQUEIO — do ambiente`: falta branch efêmera, credencial, `supabase start`, dado de seed. Destravável pelo orchestrador/CTO, **não** pelo engenheiro.
- `BLOQUEIO — herdado`: já não rodava antes da branch. **Não é destravável nesta task.** Diga isso explicitamente e recomende `PASSA — parcial` no que deu pra exercitar, ou escalar.

Sem essa classificação o orchestrador re-despacha, bloqueia de novo, e o ciclo não tem teto.
...
## Falhas (se FALHA)
1. <passos de repro — o que fez — o que esperava — o que aconteceu>   ← só comportamento QUE O DIFF MUDOU

## Quebrado antes (não bloqueia)
- QUEBRADO-ANTES — <fluxo> — <como confirmei que já falhava no base>
...
- **Escopo = comportamento que o diff mudou.** Já quebrado antes = `QUEBRADO-ANTES`, não FALHA. Na dúvida, teste no base.
- `FALHA — bloqueado` sempre classificada: `do diff` / `do ambiente` / `herdado`. Sem classe, o orchestrador re-despacha em círculo.
...
| FALHA por fluxo que já era quebrado antes da branch | `QUEBRADO-ANTES` + como confirmou no base. Vira issue, não volta |
| `FALHA — bloqueado` sem dizer de quem é o bloqueio | Classifique: do diff / do ambiente / herdado |
| Ler `✖ N problems` do eslint como vermelho | `npm run lint:ratchet` dá o veredito delta. `eslint .` cru imprime 29k warnings herdados e sai 0 |
