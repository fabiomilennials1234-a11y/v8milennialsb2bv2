...
**10. The Copilot reads the Negócio's position, not the Lead's.** `decide-action.ts` resolves the current stage from `leads.pipe_whatsapp`, a denormalized column that is the Lead holding a Stage — forbidden by decision 1, and already unreliable (**1.885 Leads in 34 orgs** where it disagrees with the card). Under decision 4 that column stops tracking the Negócio, so leaving this alone would silently change the agent's behaviour on the pilot's first day. The column survives as a legacy mirror and is dropped in a later slice.

> **Correction of record, 2026-08-03 — this decision's stated mechanism was wrong, and the real one is worse.**
> The sentence above originally read *"a trigger nulls that column when a Negócio leaves Oportunidades"*. Measured against the trigger in production (`trg_sync_whatsapp_stage_to_lead` → `sync_pipeline_entry_to_lead_pipe_whatsapp`): it resolves the funnel slug from **`NEW.pipeline_id`** on INSERT/UPDATE and from `OLD.pipeline_id` on DELETE. It nulls the column **only on the DELETE branch** — and decision 4 replaces that delete with a move. On the move, `NEW.pipeline_id` is already Orçamentos, the slug is no longer `whatsapp`, and the function **writes nothing at all**. `leads.pipe_whatsapp` does not go empty; it **freezes at the last WhatsApp stage, permanently**.
>
> Stale is worse than empty, and it inverts the failure mode. Empty degrades safely — no stage means no kanban rule matches and no template variable renders. Frozen actively lies: `{estagio}` keeps printing `compareceu` after the Negócio has moved on, and an automation condition comparing stage matches **always** instead of never. The five workflow readers named as an L2 prerequisite are therefore not a silent no-op risk but a wrong-send risk, and the priority goes up accordingly.
>
