...
**Nota:** `execution/typescript/business/create_follow_ups.ts` usa a tabela `follow_ups` (SDR/Ações do Dia), **não** `copilot_agent_followup_rules`. São sistemas diferentes.
...
**Problema:** O AgentEngine usa `conversation_context_summary` para contexto de follow-up. Se a tabela não existir em algumas instalações, `loadConversationContext` pode falhar ou retornar contexto vazio.
...
- Monta seções claras: identidade, personalidade, objetivo, contexto, FAQs, capabilities
- Injeta dados do lead e contexto da última conversa
- Instruções finais (transparência, ética, transferência humana)
