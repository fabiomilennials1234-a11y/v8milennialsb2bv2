...
## Atualização R14 — credenciais e Copilot

Documentação oficial de `/message/download` exige chave OpenAI válida; `/instance/status` confirmou `openai_apikey` vazio na TorqueSDR, ainda conectada. Isso identifica um pré-requisito ausente; não prova defeito interno no cache. Nenhuma chave alterada. QA também não tem OPENROUTER_API_KEY nem agente ativo.

`agent-message` agora exige credencial de IA somente ao iniciar um turno elegível. Antes retornava500 antes dos gates, inclusive sem agente ativo. Testes do handler comprovam401 sem autorização,400 sem organização,200 sem agente, preservação do workflow lead_replied e auto-create opt-in,500 para turno elegível sem chave e execução com cliente configurado. Deploy somente QA. Chamada externa com service_role fornecida pela API de gerenciamento retornou401 na guarda interna, inclusive antes de validar tenant; sucesso ao vivo desse caminho ainda não comprovado. Guarda preservada.

23 testes passaram em três arquivos. Typecheck Deno do Copilot continua com31 diagnósticos, idênticos ao HEAD anterior; não está verde. Mock de logEvent da suíte batch corrigido para retornar Promise. Fluxo Copilot com resposta real e credenciais permanece pendente.
