...
5. **P2 — produto:** composer de localização/contato; bloqueio/desbloqueio com UX e autorização; presença; erros de webhook; etiquetas, arquivar/silenciar/fixar conversa, perfil/privacidade e respostas rápidas após definir fonte de verdade CRM versus WhatsApp.
...
| `POST /send/contact` — Enviar cartão de contato (vCard) | **Backend integrado (escopo parcial)**. Adicionado provider/proxy; rejeita múltiplos contatos/emails sem truncar. | Aceitação real + validações negativas. | Controle dedicado no compositor; homologar todos os campos visuais. P1 — concluir homologação CRM. |
| `POST /send/location` — Enviar localização geográfica | **Backend integrado (escopo parcial)**. Adicionado provider/proxy com validação. | Coordenadas válidas aceitas; (0,0) recusado pelo fornecedor. | Controle dedicado no compositor e divergência (0,0). P1 — concluir homologação CRM. |
| `POST /message/presence` — Enviar atualização de presença | **Backend integrado (escopo parcial)**. Método existente no client. | Sem homologação real conclusiva registrada. | Validar estados e expiração; integração visual de presença. P1 — concluir homologação CRM. |
...
| Operação / finalidade | Implementação e sessão | Validação | Falta / prioridade |
|---|---|---|---|
| `POST /message/download` — Baixar arquivo de uma mensagem | **Backend integrado (escopo parcial)**. base64Data + persistência de mídia; transcrição autenticada com cache/lease. | Downloads reais; transcrição HTTP200 persistida após reload; outsider403; concorrência200/409. | Áudio recebido real; falha entre cobrança do fornecedor e persistência não é exactly-once. P1 — concluir homologação CRM. |
...
| `POST /message/markread` — Marcar mensagens como lidas | **Backend integrado (escopo parcial)**. Contrato existente preservado e testado. | Ação sobre mensagem própria aceita; receipts de conversa real observados separadamente. | Confirmar markread de entrada real no telefone remoto. P1 — concluir homologação CRM. |
...
| Operação / finalidade | Implementação e sessão | Validação | Falta / prioridade |
|---|---|---|---|
| `POST /chat/block` — Bloqueia ou desbloqueia contato do WhatsApp | **Backend integrado (escopo parcial)**. Adicionado provider/proxy. | Sem bloqueio real de contato. | Ação UI + bloquear/desbloquear número controlado. P1 — concluir homologação CRM. |
| `GET /chat/blocklist` — Lista contatos bloqueados | **Backend integrado (escopo parcial)**. Adicionado provider/proxy. | HTTP200 nesta rodada, consulta somente leitura; ver evidência R10. | Lista não prova funcionamento da mutação de bloqueio. P1 — concluir homologação CRM. |
