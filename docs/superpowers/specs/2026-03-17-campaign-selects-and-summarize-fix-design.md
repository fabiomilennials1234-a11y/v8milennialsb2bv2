...
O editor visual de workflows tem ações e triggers de campanha que usam `<Input>` (texto livre com UUID) onde deveriam usar `<Select>` dinâmicos carregados do banco. Além disso, a ação "Resumir Conversa I.A." tem um bug crítico que impede seu funcionamento.
...
### Cascade-clear: limpar seleções dependentes
