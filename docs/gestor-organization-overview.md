...
- 13 testes de SQL executados em PostgreSQL embarcado (PGlite), com papéis reais,
  limites de período, estornos, presença, isolamento e revogação. Incluem o gate
  da RPC master original. Comando: `node --test tests/integration/gestor-organization-overview.test.mjs`.
- 49 testes Vitest: página, consulta, identidade, guard de gestor, cargo/admin,
...
A migration de indicadores e as alterações de frontend ainda não foram
publicadas em produção. A atualização acima é somente dos cadastros solicitados.

## Publicação

Aplicar `20260916181815_gestor_organization_overview.sql` antes do frontend.
...
Após aplicar, regenerar os tipos Supabase para substituir a ponte tipada local.
