...
12. **follow_up_automations** - Automações de follow-up
...
- `usePipeConfirmacao()` - ✅ Seleciona confirmações com joins
...
- `useCreatePipeConfirmacao()` - ✅ Insere confirmações e dispara automações
...
- Operações transacionais (campanha + stages + members)
...
- `useUpdatePipeConfirmacao()` - ✅ Atualiza confirmações e dispara automações
...
- `useDeletePipeConfirmacao()` - ✅ Deleta confirmações
...
**Status:** ✅ Real-time configurado para atualizações automáticas
...
- ⚠️ **Verificar:** Políticas RLS no Supabase podem estar bloqueando operações
...
3. ✅ **CRUD:** Todas as operações básicas implementadas
