...
A primeira migration foi aplicada **parcialmente**. Apenas os **enums** foram criados, mas as **tabelas, funções, triggers e políticas RLS** não foram criadas.
...
3. **Permissões:** Pode haver limitações de permissão para criar certos objetos
...
- **Funções adicionais** (`has_role`, `is_team_member`, `handle_new_user`)
