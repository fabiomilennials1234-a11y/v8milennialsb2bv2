...
| loop_limit | int | Teto global de iterações de loop (default: 100) |
...
│   ├── useWorkflowExecution.ts       # Consulta execuções e steps
...
### Navegação
- Item próprio "Automações" no sidebar principal

### Fluxo de Interação
1. `/automacoes` → lista com cards/tabela (nome, status ativo/inativo, última execução, total execuções)
...
### Limites
- **Por nó**: configurável pelo usuário na edge de loop (campo "máximo de repetições")
...
| 5 | Item "Automações" próprio no sidebar | Config, substituir follow-ups | Visibilidade justifica |
...
| 11 | Condições avaliam qualquer dado | Conjunto fixo | Flexibilidade para cenários complexos |
...
17. Implementar ações (WhatsApp, mover estágio, tags, follow-up, copilot handoff)
18. Implementar lógica de condições (operadores + acesso a dados)
19. Implementar controle de loops (contadores + limites)
20. Criar hook `useWorkflowExecution.ts` (consulta execuções)
