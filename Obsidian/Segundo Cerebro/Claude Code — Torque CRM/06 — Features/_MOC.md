---
type: feature
title: 06 — Features — Map of Content
status: active
created: 2026-05-15
updated: 2026-05-15
tags: [moc, auto-regenerated]
related: []
owner: claude-agent
---

# 06 — Features — Map of Content

> Auto-gerado por `scripts/vault-regen-indexes.mjs`.
> Seções marcadas `<!-- manual -->` são preservadas.

## Admin

- [[Permissoes Sistema|Permissões Sistema]]

## Chat

- [[chat-bubble-instance-filter|Chat Bubble — Filtro de Instancia]]
- [[chat-bubble|Chat Bubble Kanban]]
- [[conversa-do-lead|Conversa do Lead — de qual numero falamos com este cliente]] (spec)
- [[disparos-falha-entrega|Disparos — Falha na Entrega por Lead]]
- [[disparos-publico-por-condicao|Disparos — Público por Condição (Todos os Funis / Todas as Etapas)]]
- [[layout-onda-2b|Chat Onda 2b — contrato de layout]]
- [[whatsapp-stability-plan|WhatsApp Stability — estado consolidado]]

## Dashboard

- [[TV Dashboard|TV Dashboard]]

## IA

- [[Auto-criar lead no inbound|Auto-criar lead no inbound]]
- [[Copilot v2 — Rebuild|Copilot v2 — Rebuild]] (design)
- [[Copilot|Copilot]]

## Infra

- [[Runbook — Cron e Webhooks|Runbook — Cron Jobs e Webhooks]]

## Suporte

- [[Anuncio de Lancamento|Central de Suporte — Anúncio de lançamento (in-app)]]

## Vendas

- [[Agenda Interna|Agenda Interna]]
- [[Carteira Pedidos|Carteira — aba Pedidos (listar e editar)]]
- [[Copiar resumo do negócio|Copiar resumo do negócio]]
- [[Export Stage Kanban|Export Stage (Kanban)]]
- [[Filtros do Kanban|Filtros do Kanban]]
- [[Lead Card|Lead Card (Kanban) — Trello-style]]
- [[Lead Detail Modal|Lead Detail Modal]]
- [[Pipe Confirmacao|Pipe Confirmacao]]
- [[Upsell|Upsell]]
- [[pipelines-auto-assign|pipelines-auto-assign]] 📦 archived

## automacoes

- [[condition-utm-value-combobox|condition-utm-value-combobox]]
- [[copy-paste-nodes|Automações — Copiar/Colar Nós]]
- [[janela-comercial-wait-business-window|janela-comercial-wait-business-window]]
- [[marcar-item-checklist|marcar-item-checklist]]
- [[negocio-criado|Automações — Trigger Negócio Criado + Node Criar Negócio]]
- [[rpc-consolidation|Automações — RPC Consolidation + Health Check]]
- [[trigger-lead-respondeu|Automações — Trigger "Lead Respondeu" (funil, etapa e número)]]

## modularizacao

- [[auditoria-duplicatas|Modularização — Auditoria de Duplicatas]] 📦 archived
- [[event-bus-plano|Modularização — Plano de Event-Bus]] 📦 archived

## whatsapp-write-instance

- [[01-schema|WhatsApp Write Instance — Schema (Etapa A)]]
- [[02-ui-states|Estados do composer — vínculo user ↔ instância de escrita]]
- [[03-frontend|Etapa C — Frontend (vínculo user ↔ instância de escrita)]]
- [[04-uat-roteiro|UAT — Roteiro de Testes Presenciais]]
