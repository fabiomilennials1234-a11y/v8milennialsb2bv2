...
Não constrói chatbots. Constrói agentes que convertem leads em reuniões.
...
**RAG Pipeline:**
- Google Gemini embeddings (1536 dimensões) + pgvector
...
- `supabase/functions/_shared/ai-action-executor.ts` — Executor de ações
...
2. **Entender fluxo completo** — Criar agente → configurar → ativar → conversar → ações
...
5. **Testar E2E** — Criar agente → config → conversar → verificar ações
...
| `superpowers:systematic-debugging` | Ao debugar conversas falhando ou ações não executando |
...
- NUNCA modificar ai-action-executor sem testar todas as ações
