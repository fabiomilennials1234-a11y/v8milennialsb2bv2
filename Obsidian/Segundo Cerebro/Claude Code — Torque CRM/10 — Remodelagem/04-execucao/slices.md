...
| 9 ✅ | `feat/modularizacao/08-campaigns` | campanhas + mass-send + templates — *concluído 2026-05-27. Frontend migrado (27 renames: 20 components + 4 hooks + 3 pages + 41 arquivos com imports reescritos, 67 substituições). Backend `_shared/campaign-distribution.ts` adiado pra slice 16. Edge functions (6) pra slice 15. `useUpsellCampanhas` NÃO migrado (entidade carteira, aguarda slice 10). `useOutboundMetrics` NÃO migrado (entidade analytics, aguarda slice 12). `MessageTemplates.tsx` NÃO migrado (cross-cutting platform, decisão slice 14). `communication.useWhatsAppChat` barrel corrigido pra usar API pública `@/modules/campaigns` em vez de deep-import* | 4h |
