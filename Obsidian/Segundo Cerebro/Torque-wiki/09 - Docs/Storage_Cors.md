...
status: active
source: docs/STORAGE_CORS.md
---

# CORS no bucket de Storage (áudio no chat)

Se a aplicação roda em um domínio diferente do Supabase (ex.: app em `https://app.seudominio.com` e Storage em `https://xxx.supabase.co`), o bucket **media** precisa permitir requisiçoes do domínio da app para que áudios do chat carreguem corretamente.
