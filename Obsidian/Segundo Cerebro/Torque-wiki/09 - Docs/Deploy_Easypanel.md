...
status: active
source: docs/DEPLOY_EASYPANEL.md
---
...
- **Port:** o container expoe a porta **80** (Nginx). No EasyPanel, configure o App para usar a porta **80** (ou a que o painel mapear, ex.: 80 → 80).
...
Se o deploy falhar com **429 Too Many Requests** e mensagem sobre "scraping GitHub", o GitHub está limitando as requisiçoes (rate limit). Acontece quando a plataforma (EasyPanel, Hostinger, etc.) faz muitas chamadas ao GitHub em pouco tempo.
...
2. **Usar Personal Access Token (recomendado)**  
   Conexoes autenticadas têm limite bem maior (milhares de requisiçoes/hora). No EasyPanel (ou na plataforma que faz o deploy):
