...
status: active
source: docs/plan-outbound-implementation.md
---
...
1. **`src/hooks/useClientSidebarPermissions.ts`** - Novo hook:
   - `useClientSidebarPermissions()`: buscar permissoes da org atual
...
2. **`src/components/layout/Sidebar.tsx`** - Lógica condicional:
   ...
   - Se `orgType === 'outbound'`:
     ...
     - `bdr`: filtro fixo (sem Equipe, Comissoes, Config, Produtos, TV)
     - `cliente`: itens fixos (sem Campanhas, Equipe, Config, Comissoes, Produtos, TV) + itens configuráveis via `client_sidebar_permissions`
