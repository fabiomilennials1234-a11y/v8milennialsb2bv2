...
status: active
source: docs/superpowers/plans/2026-03-18-analytics-phase1.md
---
...
export function RadarComparison({ members }: Props) {
  ...
  const radarData = [
    ...
    // data from Phases 2-3. Using Vendas/Reunioes/Propostas as Phase 1 proxy.
    { axis: "Vendas", ...Object.fromEntries(selected.map((id) => {
      const m = members.find((x) => x.member_id === id)!;
      return [id, normalize(m.deals_won, maxes.velocity)];
    }))},
    { axis: "Reunioes", ...Object.fromEntries(selected.map((id) => {
      const m = members.find((x) => x.member_id === id)!;
