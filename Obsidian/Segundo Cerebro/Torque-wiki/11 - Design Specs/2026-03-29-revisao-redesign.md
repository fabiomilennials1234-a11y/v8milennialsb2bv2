...
status: active
source: docs/superpowers/specs/2026-03-29-revisao-redesign.md
---
...
Reformulação completa da página Revisão como um checklist unificado de tarefas. Mescla follow-ups e mensagens agendadas numa interface tipo Todoist - compacta, com tabs para filtrar, açoes contextuais ao completar, e sugestoes inteligentes. Reativa a aba Revisão na navegação.
...
2. Lista plana ordenada cronologicamente por data/hora - sem agrupamento por seçoes
3. Cada item é uma linha compacta: checkbox + título + badge tipo + lead + data + prioridade
4. Click no item expande inline: descrição, açoes rápidas (WhatsApp, Editar, Cancelar)
...
8. Sugestoes inteligentes no topo da tab "Tudo" (leads sem ação, follow-ups vencidos, leads quentes)
...
- Permissoes existentes mantidas: `followups.view`, `followups.delete`
...
Sugestoes colapsáveis (tab "Tudo" apenas)
...
- `useDailyPriorities()` - existente (sugestoes)
...
| `src/components/revisao/RevisionItemExpanded.tsx` | Conteúdo expandido com açoes |
| `src/components/revisao/CompletionBanner.tsx` | Banner "Enviar mensagem?" pós-completar |
| `src/components/revisao/SuggestionsSection.tsx` | Sugestoes inteligentes colapsáveis |
...
- AutomationSettings (admin configura automaçoes) - acessível via botão na página, modal existente mantido
- Drag-and-drop para reordenar tasks
- Agendamento recorrente
- Notificaçoes push para tasks vencidas
