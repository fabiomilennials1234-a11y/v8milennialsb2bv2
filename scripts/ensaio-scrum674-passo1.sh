#!/usr/bin/env bash
# scripts/ensaio-scrum674-passo1.sh — ensaio transacional do passo 1 da
# SCRUM-674 (as invariantes viram função compartilhada) contra PRODUÇÃO.
...
#   / 20270930000000 (ARQUIVO DE VERDADE, sem transação própria)
#   / depois (igualdade view-vs-função + controle positivo + RAISE ENSAIO_OK)
#   / ROLLBACK. Nada é aplicado.
#
# Uso:
#   scripts/ensaio-scrum674-passo1.sh --montar   # só monta e imprime o caminho
#   scripts/ensaio-scrum674-passo1.sh            # roda contra prod (aborta sozinho)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MIG="$ROOT/supabase/migrations/20270930000000_as_invariantes_viram_funcao.sql"
ANTES="$ROOT/scripts/ensaio-scrum674-passo1.sql"
DEPOIS="$ROOT/scripts/ensaio-scrum674-passo1-depois.sql"
...
OUT="${ENSAIO_OUT:-$ROOT/.ensaio-scrum674-passo1.montado.sql}"
...
echo "==> ordem: antes -> 20270930000000 -> depois (ENSAIO_OK) -> ROLLBACK"
