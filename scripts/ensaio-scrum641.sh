#!/usr/bin/env bash
# scripts/ensaio-scrum641.sh — ensaio transacional do SCRUM-641 (org nova
...
#
# Uso:
#   scripts/ensaio-scrum641.sh --montar   # só monta e imprime o caminho
#   scripts/ensaio-scrum641.sh            # roda contra produção (aborta sozinho)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MIG="$ROOT/supabase/migrations/20270918000000_org_nova_nasce_com_funil_de_vendas.sql"
ANTES="$ROOT/scripts/ensaio-scrum641.sql"
DEPOIS="$ROOT/scripts/ensaio-scrum641-depois.sql"
...
OUT="${ENSAIO_OUT:-$ROOT/.ensaio-scrum641.montado.sql}"
...
grep -q "ENSAIO_OK SCRUM-641" "$DEPOIS" || { echo "RECUSADO: depois sem RAISE ENSAIO_OK" >&2; exit 1; }

echo "==> ensaio montado em $OUT ($(wc -l < "$OUT" | tr -d ' ') linhas)"
echo "==> ordem: antes -> 20270918000000 -> depois (ENSAIO_OK) -> ROLLBACK"
