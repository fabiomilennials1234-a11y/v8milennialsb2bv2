 ...
 * Windows variant: reads SUPABASE_ACCESS_TOKEN from .env.local (macOS keychain unavailable).
 *
 * Usage:
 *   node scripts/prod-sql-win.mjs "SELECT 1"
 *   node scripts/prod-sql-win.mjs --file path/to.sql
 */
...
if (!arg) {
  console.error('usage: node scripts/prod-sql-win.mjs "<sql>" | --file <path>');
  process.exit(1);
}
