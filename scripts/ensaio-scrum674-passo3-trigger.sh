#!/usr/bin/env bash
# Ensaio transacional da janela 2 do passo 3 da SCRUM-674 contra PROD.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MIG="$ROOT/supabase/migrations/20271005000000_o_trigger_sai_dos_espelhos.sql"
ANTES="$ROOT/scripts/ensaio-scrum674-passo3-trigger.sql"
DEPOIS="$ROOT/scripts/ensaio-scrum674-passo3-trigger-depois.sql"
ROLLBACK_MIG="$ROOT/supabase/migrations/rollback/20271005000000_o_trigger_sai_dos_espelhos.sql"
ROLLBACK_ANTES="$ROOT/scripts/ensaio-scrum674-passo3-trigger-rollback-antes.sql"
ROLLBACK_DEPOIS="$ROOT/scripts/ensaio-scrum674-passo3-trigger-rollback-depois.sql"
OUT="${ENSAIO_OUT:-$ROOT/.ensaio-scrum674-passo3-trigger.montado.sql}"
...
if [[ "$MODO" == "rollback" ]]; then
  ...
  MARCADOR="ENSAIO_OK SCRUM-674 rollback janela 2"
  ORDEM="baseline PROD -> migration -> rollback dedicado -> comparação -> ROLLBACK"
else
  cat "$ANTES" "$MIG" "$DEPOIS" > "$OUT"
  MARCADOR="ENSAIO_OK SCRUM-674 passo 3 janela 2"
  ORDEM="trigger velho -> migration -> trigger novo -> comparação -> ROLLBACK"
fi
