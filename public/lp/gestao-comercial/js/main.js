...
(function () {
  ...
  (function () {
    ...
    var STEPS = [
      { t: "Novo lead", rows: [["Gatilho", "lead entrou"], ["Ação", "distribuir para SDR"], ["Regra", "rodízio · SLA 5 min"]] },
      { t: "Sem contato", rows: [["Gatilho", "3 dias sem contato"], ["Ação", "criar tarefa + mensagem"], ["Para", "vendedor responsável"]] },
      { t: "Qualificou", rows: [["Gatilho", "IA classificou"], ["Ação", "mover no funil"], ["Etapa", "Reunião"]] },
      { t: "Agendou", rows: [["Gatilho", "reunião marcada"], ["Ação", "avisar o Closer"], ["Extra", "lembrete 1h antes"]] },
      { t: "Não compareceu", rows: [["Gatilho", "no-show"], ["Ação", "iniciar recuperação"], ["Sequência", "D+0 · D+2 · D+5"]] },
      { t: "Proposta parou", rows: [["Gatilho", "proposta sem resposta"], ["Ação", "iniciar follow-up"], ["Sequência", "+2 · +5 · +10 dias"]] }
    ];
  ...
  var MOCKS = {
    vendors: function (root) {
      var tabs = $$("[data-vtabs] span", root), panels = $$("[data-vt]", root);
      function show(k) { tabs.forEach(function (t, i) { t.classList.toggle("on", i === k); }); panels.forEach(function (p, i) { p.hidden = i !== k; }); }
      var tl = gsap.timeline({ repeat: -1 });
      [0, 1].forEach(function (k) {
        tl.add(function () { show(k); var rows = $$("[data-vrow]", panels[k]); rows.forEach(function (r) { r.classList.remove("is-hi"); }); gsap.fromTo(rows, { x: -10, opacity: 0 }, { x: 0, opacity: 1, duration: 0.35, stagger: 0.07 }); })
          .add(function () { var rows = $$("[data-vrow]", panels[k]); rows[0].classList.add("is-hi"); }, "+=1.0")
          .add(function () { var rows = $$("[data-vrow]", panels[k]); rows[0].classList.remove("is-hi"); rows[rows.length - 1].classList.add("is-hi"); }, "+=1.4")
          .add(function () { $$("[data-vrow]", panels[k]).forEach(function (r) { r.classList.remove("is-hi"); }); }, "+=1.4")
          .add(function () {}, "+=0.4");
      });
      return tl;
    },
    rules: function (root) {
      var items = $$("[data-rule]", root);
      var tl = gsap.timeline({ repeat: -1, repeatDelay: 3 });
      tl.fromTo(items, { x: -10, opacity: 0 }, { x: 0, opacity: 1, duration: 0.35, stagger: 0.08 })
        .add(function () { $$("[data-count]", root).forEach(function (el) { countUp(el, +el.dataset.count); }); }, 0.2);
      items.forEach(function (it, i) { tl.add(function () { items.forEach(function (x) { x.classList.remove("is-hi"); }); it.classList.add("is-hi"); }, "+=" + (i === 0 ? 0.6 : 0.5)); });
      tl.add(function () { items.forEach(function (x) { x.classList.remove("is-hi"); }); }, "+=0.6");
      return tl;
    },
    base: function (root) {
      var tl = gsap.timeline({ repeat: -1, repeatDelay: 3 });
      tl.fromTo($$("[data-seg]", root), { opacity: 0, y: 8 }, { opacity: 1, y: 0, duration: 0.4, stagger: 0.1 })
        .fromTo($$(".bar i", root), { scaleX: 0 }, { scaleX: 1, duration: 0.7, stagger: 0.08, ease: "power3.out" }, "-=0.2")
        .add(function () { $$("[data-count]", root).forEach(function (el) { countUp(el.querySelector("span") || el, +el.dataset.count); }); }, "-=0.4");
      return tl;
    },
  ...
  var LEAD_LP_ID = "gestao-comercial";
  var CAL_AGENDAS = [
