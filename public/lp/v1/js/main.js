/* ============================================================
   TORQUE · Variação 01 — Interações
   GSAP + ScrollTrigger (CDN) + product preview interativo
   ============================================================ */
(function () {
  "use strict";
  ...
  /* ============================================================
     1 · CANVAS DE FUNDO DO HERO — partículas leves
     ============================================================ */
  function initHexCanvas() {
    ...
    var pCount = window.innerWidth < 760 ? 18 : 36;
    var particles = [];
  ...
  /* ============================================================
     2 · Navegação — estado de scroll
     ============================================================ */
  var nav = document.getElementById("nav");
  var lastScrolled = false;
  window.addEventListener("scroll", function () {
    var y = window.scrollY;
    var s = lastScrolled ? y > 24 : y > 72; // histerese: liga em 72px, desliga só abaixo de 24px
    if (s !== lastScrolled) {
      lastScrolled = s;
  ...
  /* ============================================================
     3 · FAQ — fecha os outros ao abrir
     ============================================================ */
  ...
  if (form) {
    ...
    // limpa o estado de erro assim que o campo é preenchido
    ...
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      // valida tudo; se houver etapa anterior incompleta, volta para ela
  ...
  /* ============================================================
     5 · GSAP — entradas, reveals, contadores, tilt
     ============================================================ */
  function initGsap() {
    if (typeof gsap === "undefined") {
      // CDN indisponível: garante que nada fica escondido
    ...
    if (prefersReduced) {
      gsap.set("[data-reveal], [data-hero-fade], .hero__title .line, .hero__preview", { clearProps: "all" });
      document.querySelectorAll(".count").forEach(function (el) {
        var v = parseFloat(el.dataset.count || "0");
        el.textContent = Math.round(v).toLocaleString("pt-BR") + (el.dataset.suffix || "");
      });
      return;
    }
    ...
    heroTl
      .fromTo(".hero__title .line",
        { y: 46, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.9, stagger: 0.09 }, 0.15)
      .fromTo("[data-hero-fade]",
        { y: 18, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.8, stagger: 0.12 }, 0.4)
      .fromTo(".hero__preview",
        { opacity: 0, scale: 0.9 },
        { opacity: 1, scale: 1, duration: 1.2, ease: "power2.out" }, 0.5);
    ...
    /* --- discadora: loop dos passos desperdiçados --- */
    var steps = document.querySelectorAll("#dialSteps li");
    if (steps.length) {
      var idx = 0;
      var stepTl = gsap.timeline({
        repeat: -1,
        scrollTrigger: { trigger: "#dialSteps", start: "top 85%", toggleActions: "play pause resume pause" }
      });
      steps.forEach(function (_, n) {
        stepTl.call(function () {
          steps.forEach(function (s, m) { s.classList.toggle("is-burn", m === n); });
        }, null, n * 0.55);
      });
      stepTl.call(function () {
        steps.forEach(function (s) { s.classList.remove("is-burn"); });
      }, null, steps.length * 0.55 + 0.4);
    }
