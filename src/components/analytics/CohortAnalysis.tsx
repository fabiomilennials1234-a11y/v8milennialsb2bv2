...
export function CohortAnalysis() {
  ...
  const cohortData = overviewData?.cohort_data ?? [];
  ...
  if (cohortData.length < 2) {
    return (
      <Card>
        ...
        <CardContent>
          <AnalyticsEmptyState
            message="Necessário pelo menos 2 meses de dados"
            detail="A análise de coorte agrupa leads por mês de criação e acompanha conversão ao longo do tempo."
          />
        </CardContent>
      </Card>
    );
          ...
          <table className="w-full text-xs">
            <thead>
              <tr>
                ...
                <th className="text-center font-medium text-muted-foreground py-1 px-1 min-w-[44px]">
                  Total
                </th>
              ...
              {cohortData.map((row) => {
                ...
                return (
                  <tr key={row.cohort_month} className="border-t border-border/30">
                    ...
                    <td className="py-1 px-1 text-center font-medium tabular-nums">
                      {row.total_customers}
                    </td>
                    {Array.from({ length: MAX_MONTHS }, (_, i) => {
