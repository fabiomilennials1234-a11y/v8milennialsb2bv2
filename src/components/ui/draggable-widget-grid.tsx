...
export interface DraggableWidgetGridProps {
	...
	cellSize?: number
	/** Fixed row height for compact metric cards. */
	rowHeight?: number
		...
		<motion.div
			...
			<motion.div
				...
				className={`relative isolate flex h-full w-full flex-col overflow-hidden rounded-[var(--widget-radius)] bg-card text-card-foreground ring-inset transition-shadow duration-300 [clip-path:inset(0_round_var(--widget-radius))] ${
					landed ? 'ring-2 ring-primary/60' : 'ring-1 ring-border'
				}`}>
	...
	useIsoLayoutEffect(() => {
		...
		const measure = () => {
			...
			const columns = Math.max(
				minColumns,
				Math.min(maxColumns, Math.floor((width + gap) / (cellSize + gap))),
			)
			...
			<div
				...
				{editable && (
					<p id={hintId} className="sr-only">
						Arraste para reorganizar. No celular, toque e segure.
						No teclado, segure Alt e use as setas.
					</p>
				)}
				<div
					ref={grid}
					...
					style={{
						...
						gridAutoRows: rowHeight ? `${rowHeight}px` : metrics.unit
					...
					{readingOrder.map((id) => {
						...
						return (
							<Widget
								...
								hintId={hintId}
								renderItem={
									renderItem as (item: WidgetItem, size: WidgetSize) => ReactNode
								}
							/>
						)
					})}
