...
export function TabMarketing({ month: dashMonth, year: dashYear }: TabMarketingProps) {
  const [selectedMonth, setSelectedMonth] = useState(dashMonth);
  const [selectedYear, setSelectedYear] = useState(dashYear);
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <KPICard title="Investimento Total" value={summary.totalInvestimentoReais} format="currency" icon={DollarSign} trend={trends.investimento} delay={0.05} />
            <KPICard title="Total de Leads" value={summary.totalLeads} format="number" icon={Users} trend={trends.leads} delay={0.1} />
            <KPICard title="Custo por Lead" value={summary.totalCustoPorLead} format="currency" icon={DollarSign} trend={trends.cpl ? { ...trends.cpl, isPositive: !trends.cpl.isPositive } : undefined} delay={0.15} />
            <KPICard title="Agendamentos" value={summary.totalAgendamentos} format="number" icon={Calendar} trend={trends.agendamentos} delay={0.2} />
            <KPICard title="Comparecimentos" value={summary.totalComparecimentos} format="number" icon={Target} trend={trends.comparecimentos} delay={0.25} />
            <KPICard title="Vendas" value={summary.totalVendas} format="number" icon={Trophy} trend={trends.vendas} delay={0.3} />
            <KPICard title="Custo por Venda" value={summary.totalCustoPorVenda} format="currency" icon={TrendingUp} trend={trends.cpv ? { ...trends.cpv, isPositive: !trends.cpv.isPositive } : undefined} delay={0.35} />
            <KPICard title="Conversão Geral" value={summary.totalConversionRate} format="percent" icon={PieChart} trend={trends.conversao} delay={0.4} />
          </div>
