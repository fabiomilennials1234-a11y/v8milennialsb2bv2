...
export function WorkflowDataGrantPanel({ actorId, workflowId, organizationId, canManage, requiredFields = ['lead.name'] }: {
  ...
  const existenceScopes = requiredFields.filter(field => field.startsWith('business.exists.'));
  const existenceLabel = existenceScopes.length ? `${existenceScopes.includes('business.exists.lifecycle') ? 'Ciclo' : ''}${existenceScopes.includes('business.exists.stage') ? `${existenceScopes.includes('business.exists.lifecycle') ? ', ' : ''}etapa` : ''}${existenceScopes.includes('business.exists.value') ? `${existenceScopes.length > 1 ? ' e ' : ''}valor` : ''} dos negócios` : null;
  const otherLabels = requiredFields.filter(field => !field.startsWith('business.exists.')).map(field => field === 'business.trigger.stage' ? 'Etapa do negócio do gatilho' : field === 'business.trigger.value' ? 'Valor do negócio do gatilho' : field === 'business.trigger.stage_elapsed' ? 'Tempo na etapa do negócio do gatilho' : field === 'business.last_won_date' ? 'Data da última venda ganha' : field === 'message.trigger.text' ? 'Texto da mensagem do gatilho' : field === 'message.period.exists' ? 'Existência de mensagem no período' : field === 'message.search.text' ? 'Busca no conteúdo de mensagens' : field === 'message.waiting.elapsed' ? 'Tempo aguardando resposta na conversa' : field === 'activity.follow_up' ? 'Follow-ups vinculados ao lead ou negócio' : field === 'product.trigger_business_item' ? 'Itens do negócio do gatilho' : field === 'product.lead_association' ? 'Associações manuais ativas do lead' : field === 'product.won_deal_history' ? 'Produtos registrados por negócio ganho' : field === 'lead.tags' ? 'Tags' : field === 'lead.origin' ? 'Origem' : isGuidedResponsibleField(field) ? GUIDED_RESPONSIBLE_FIELDS[field].label : isGuidedScalarField(field) ? GUIDED_SCALAR_FIELDS[field].label : customLabels.get(field.slice('lead.custom:'.length)) ?? 'Campo personalizado');
  const scopeLabel = [...(existenceLabel ? [existenceLabel] : []), ...otherLabels].join(' e ');
