...
export function useCustomFieldCatalogue(actorId: string, organizationId: string, search: string, open: boolean, selectedId?: string) {
  ...
  const options = useQuery({
    queryKey: ['lead-custom-fields', organizationId, 'catalogue', actorId, 'text-number-boolean-date', search],
    enabled: enabled && open,
    queryFn: async ({ signal }) => {
      let query = supabase.from('lead_custom_fields').select('id, field_name, field_type')
        .eq('organization_id', organizationId).in('field_type', ['text', 'number', 'boolean', 'date']).order('field_name').order('id').limit(25).abortSignal(signal);
