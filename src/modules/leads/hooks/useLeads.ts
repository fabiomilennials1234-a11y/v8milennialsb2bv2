...
export function useLeads(params: LeadsFilterParams = {}) {
  const { page = 0, searchQuery, filterOrigin, filterQualification, filterClassificacao, filterUf, createdFrom, createdTo, filterAssignment, filterResponsible, sort = DEFAULT_LEAD_SORT } = params;
  ...
  return useQuery({
    ...
    queryKey: ["leads", organizationId, page, searchQuery, filterOrigin, filterQualification, filterClassificacao, filterUf, createdFrom, createdTo, filterAssignment, filterResponsible, sort.key, sort.direction],
    queryFn: async () => {
      if (!organizationId) {
      ...
      query = applyLeadsFilters(query, organizationId, { searchQuery, filterOrigin, filterQualification, filterClassificacao, filterUf, createdFrom, createdTo, filterAssignment, filterResponsible });
...
export function useLeadsCount(filters: Omit<LeadsFilterParams, "page"> = {}) {
  const { searchQuery, filterOrigin, filterQualification, filterClassificacao, filterUf, createdFrom, createdTo, filterAssignment, filterResponsible } = filters;
  const { organizationId, isReady } = useOrganization();

  return useQuery({
    queryKey: ["leads-count", organizationId, searchQuery, filterOrigin, filterQualification, filterClassificacao, filterUf, createdFrom, createdTo, filterAssignment, filterResponsible],
    queryFn: async () => {
