...
import type { SampleSlice } from "@/modules/analytics/lib/metrics-studio-sample";
...
export function StudioPieChart({ slices, unit, compact }: StudioPieChartProps) {
      ...
      <div className="h-full min-w-0 flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            >
              {slices.map((slice, i) => (
                <Cell key={slice.label} fill={STUDIO_PALETTE[i % STUDIO_PALETTE.length]} />
              ))}
            </Pie>
          </PieChart>
        </ResponsiveContainer>
